...
describe("findAsarArchivePath", () => {
  it("resolves the containing archive for a path inside an asar", () => {
    expect(
      findAsarArchivePath(
        "/Applications/Luminor.app/Contents/Resources/app.asar/apps/server/dist/client",
      ),
    ).toBe(path.join("/Applications/Luminor.app/Contents/Resources/app.asar"));
  });
  ...
  it("returns null for plain directories", () => {
    expect(findAsarArchivePath("/Users/me/dev/luminor/apps/web/dist")).toBeNull();
  });
});
