...
describe("windowsProcess", () => {
  ...
  it("uses process.cwd for current-directory filtering when cwd is omitted", () => {
    vi.spyOn(process, "cwd").mockReturnValue("C:\\projects\\luminor");
    const spawnSync = vi.fn(() => ({
      stdout: [
        "C:\\projects\\luminor\\codex.cmd",
        "C:\\Users\\test\\AppData\\Roaming\\npm\\codex.cmd",
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      "C:\\Windows\\System32\\where.exe",
      ["codex"],
      expect.objectContaining({ cwd: "C:\\projects\\luminor" }),
    );
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      const root = mkdtempSync(Path.join(tmpdir(), "luminor-windows-process-"));
