  ...
  it("matches expected fields independent of order and extra certificate fields", () => {
    expect(
      matchesDistinguishedName(
        ...
        "C=US, O=Acme Tools, CN=Luminor, SERIALNUMBER=1234",
      ),
    ).toBe(true);
  });

  it("rejects incomplete pins and mismatched signer fields", () => {
    expect(matchesDistinguishedName("CN=Luminor", "CN=Luminor, O=Acme Tools")).toBe(false);
    expect(matchesDistinguishedName("CN=Luminor, O=Acme Tools", "CN=Luminor, O=Other Tools")).toBe(
      false,
    );
  });
});
