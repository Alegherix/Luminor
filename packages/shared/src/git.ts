 ...
 */
export const WORKTREE_BRANCH_PREFIX = "luminor";
...
const LUMINOR_BRANCH_FALLBACK = "update";
...
export function buildLuminorBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || LUMINOR_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueLuminorBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildLuminorBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
