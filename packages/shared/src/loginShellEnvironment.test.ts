...
function makeTemporaryRoot(): string {
  const root = FS.mkdtempSync(Path.join(OS.tmpdir(), "luminor-login-shell-"));
...
describe("createCachedLoginShellEnvironmentReader", () => {
  ...
  it("runs uncached when the home directory cannot be resolved", () => {
    ...
    // call-count assertion below) instead of touching the developer's real Luminor home.
    const env = { LUMINOR_HOME: Path.dirname(Path.dirname(fixture.cachePath)) };
...
describe("loginShellEnvironmentCachePath", () => {
  it("anchors the cache in the Luminor home both processes resolve", () => {
    expect(loginShellEnvironmentCachePath({ env: { LUMINOR_HOME: "/tmp/luminor-home" } })).toBe(
      Path.join("/tmp/luminor-home", "cache", "login-shell-environment.json"),
    );
    expect(loginShellEnvironmentCachePath({ env: {}, homeDirectory: "/users/test" })).toBe(
      Path.join("/users/test", ".luminor", "cache", "login-shell-environment.json"),
    );
  });
});
