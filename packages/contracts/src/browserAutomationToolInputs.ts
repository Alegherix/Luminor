...
export const BROWSER_FIELD_INSTRUCTION_COPY = {
  ...
  idempotencyKey:
    "Optional advanced retry key. Luminor derives a stable key from the authenticated tool request when omitted; provide one only to deliberately deduplicate a byte-identical retry.",
  target:
  ...
  waitUntil:
    "Navigation milestone; domcontentloaded is the default, while networkidle uses Luminor's bounded tracker.",
  annotationId:
