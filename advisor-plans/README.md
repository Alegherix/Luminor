# Luminor Roadmap Execution Controller
...
- `P2-ACP-01`: the official TypeScript SDK owns framing, JSON-RPC dispatch/correlation, validation,
  ...
  and workspace package are deleted. `AcpErrors.ts` and `AcpExtensions.ts` retain only Luminor-owned
...
- `PRUNE-09` — `ProviderService` no longer reads `LUMINOR_PROVIDER_RUNTIME_IDLE_STOP_MS` twice through
...
cancellation, handler dispatch, and NDJSON encoding. Luminor retains Effect lifecycle, bounded
