...
**A review of the Luminor codebase found an analytics configuration that came from the original T3 Code codebase when Luminor was created as a clone in March. We did not add it, and we have no access to the PostHog project receiving the events.**

**The configuration has been removed. Luminor no longer sends remote product analytics. The events did not include prompts, source code, filenames, or file contents.**
...
- Bumped Luminor release package versions to `0.6.7` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Fixed foreground completion notifications appearing while Luminor or its native browser pane already had the user's attention, and aligned toast visibility with side-chat split and dock routes.
...
- Bumped Luminor release package versions to `0.6.6` across the server, desktop, web, and contracts packages.
...
- Bumped Luminor release package versions to `0.6.5` across the server, desktop, web, and contracts packages.
...
- Bumped Luminor release package versions to `0.6.4` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Bumped Luminor release package versions to `0.6.3` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Bumped Luminor release package versions to `0.6.2` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Fixed desktop renderer crashes that could leave Luminor blank instead of recovering within a bounded retry policy.
...
- Bumped Luminor release package versions to `0.6.1` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Added agent-facing MCP tools for creating, suggesting, listing, viewing, replacing, pausing, deleting, remembering, and reporting results from Luminor automations.
...
- Added a one-prompt external setup flow for agentic clients with resumable pairing, automatic local stdio configuration, connection verification, and the exact executable and Luminor data directory from the running installation.
...
- Added cross-task composer mentions that attach bounded recent transcript context from another Luminor task with its project and provider identity.
...
- Added an isolated Luminor Canary workflow for clean-checkout desktop testing and attachment uploads.
- Added a Studio folder row that opens the selected folder in the platform file manager.

### Changed

- Agents now receive explicit Luminor operating guidance: when to delegate parallel work, wait for every requested result, prefer Luminor diagnostics over raw database inspection, respect worktree and full-access boundaries, and suggest rather than silently enable automations.
- Agent-created and externally created work remains ordinary standalone Luminor tasks with visible origin, independent lifecycle, and results that users and other agents can follow.
...
- Live and attention-needing tasks receive clearer sidebar priority, while cross-task attribution is simplified to a single Luminor label.
...
- Bumped Luminor release package versions to `0.6.0` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.

### Fixed

- Fixed Luminor browser-control discovery and desktop browser RPC negotiation, session ownership, teardown, reconnect, and fixture readiness.
- Fixed blank provider `PATH` defaults being rejected or replaced incorrectly.
- Fixed MCP `serve` and `pair` ignoring `--home-dir`, which could connect an integration to the wrong Luminor data directory.
...
- Fixed External MCP documentation omitting the primary `luminor_overview` discovery tool and describing a superseded client-picker setup flow.
...
- Fixed `luminor_list_projects` exposing system-managed Chat, Studio, and legacy Home containers as ordinary projects.
...
- Fixed OpenCode `/review` being forwarded as plain text instead of opening Luminor's review flow.
...
- Fixed Antigravity's global capture hook launching the Luminor GUI outside active sessions.
...
- Fixed missing project directories being reported as "Codex not installed"; Luminor now identifies the missing working directory and offers relocation guidance.
...
- Replaced Pierre-branded side-panel diff headers with Luminor's shared visual chrome.
...
- Bumped Luminor release package versions to `0.5.5` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Fixed stable updater feeds to publish both GitHub Latest metadata and the `luminor-*` channel aliases expected by installed desktop builds.
...
- Completed the Luminor identity cutover across desktop packaging, the renderer origin, workspace packages, the public CLI, runtime variables, storage, Git metadata, assets, documentation, and release automation.
- Set the production bundle ID and Windows AUMID to `com.emanueledipietro.luminor`, with `.dev` used only for development.
- Published the CLI identity as `@luminor/cli` with the `luminor` executable and moved all first-party workspaces to `@luminor/*`.
...
- Final full `bun run test` passed: 10 Turbo tasks; `@luminor/web` passed 200 files / 2,426 tests, and `@luminor/cli` passed 152 files / 1,698 tests with 1 skipped file and 6 skipped tests. The initial run was interrupted while waiting on the serial server suite; the final rerun completed cleanly.

### Upgrade note

- Launch Luminor 0.4.2 once before upgrading so renderer-local UI state is exported before 0.5.0 adopts the canonical `luminor://app` origin.

## 0.4.2 - 2026-07-09

### Added

- Added the Luminor identity bridge that exports canonical renderer storage before the packaged origin changes.
...
- Enforced the staged Luminor update feed end to end, with fail-closed preflight checks in the release pipeline.
- Made Windows code signing optional in the release pipeline and finalized Luminor license attribution.
...
- Launch Luminor 0.4.2 at least once before installing the next release. This preserves drafts, pins, theme, browser state, and other local UI state through the identity cutover.
...
- Bumped Luminor release package versions to `0.4.1` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Bumped Luminor release package versions to `0.4.0` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Full `bun run test` passed: 10 tasks successful in 6m35.477s. `@luminor/web` passed 194 files / 2352 tests, and `luminor` passed 145 files with 1 skipped file, 1593 passed tests, and 6 skipped tests.
- `bun install` refreshed `bun.lock` after the package-version bump and reported no dependency changes.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.4.0`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.9` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Full `bun run test` passed: 10 tasks successful in 6m35.955s. `@luminor/web` passed 193 files / 2316 tests, and `luminor` passed 144 files with 1 skipped file, 1575 passed tests, and 6 skipped tests.
- `bun install` refreshed `bun.lock` after the package-version bump and reported no dependency changes.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.9`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.8` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- Initial full `bun run test` failed in `@luminor/web` with one timeout: `apps/web/src/components/ChatMarkdown.test.tsx > ChatMarkdown > uses the theme foreground token for markdown text`. No stale duplicate test processes were present; the targeted rerun `bun run test src/components/ChatMarkdown.test.tsx -t "uses the theme foreground token for markdown text"` from `apps/web` passed in 1.01s.
- Final full `bun run test` passed: 10 tasks successful in 9m28.476s. `@luminor/web` passed 193 files / 2308 tests, `luminor` passed 140 files with 1 skipped file, 1547 passed tests, and 6 skipped tests.
- `bun install` refreshed `bun.lock` after the package-version bump and reported no dependency changes.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.8`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.7` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace metadata.
...
- `bun run test` passed: 10 tasks successful in 5m23.405s. `@luminor/web` passed 191 files / 2274 tests. `effect-acp` passed 3 files / 24 tests. `luminor` passed 140 files with 1 skipped file, 1532 passed tests, and 6 skipped tests.
- `bun install` refreshed `bun.lock` after the package-version bump and reported no dependency changes.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.7`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.6` across the server, desktop, web, and contracts packages.
...
- `bun run test` passed: 10 tasks successful in 5m38.929s. `@luminor/web` passed 191 files / 2273 tests. `luminor` passed 138 files with 1 skipped file, 1517 passed tests, and 6 skipped tests.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.5` across the server, desktop, web, and contracts packages, and refreshed `bun.lock` workspace package versions.
...
- `bun run build` passed. The build still reports existing Astro `transformWithEsbuild`, tsdown/plugin timing, desktop typeless-module, and large Vite chunk warnings.
- `bun run test` passed: 10 tasks successful in 6m9.469s. `@luminor/web` passed 190 files / 2229 tests. `luminor` passed 137 files with 1 skipped file, 1492 passed tests, and 6 skipped tests.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.5`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.4` across the server, desktop, web, and contracts packages.
...
- `bun run fmt:check` passed.
- `bun run lint` passed with 155 warnings, 0 errors.
- Initial `bun run typecheck` failed in `@luminor/web` on `apps/web/src/components/ChatView.browser.tsx` because a browser `scrollTo` test helper produced explicit `undefined` optional fields; after that fix it failed in `luminor` on `apps/server/src/provider/Layers/ProviderHealth.ts` and `ProviderHealth.test.ts` for the same exact-optional pattern and an unsupported Effect platform error tag; after targeted fixes, `bun run typecheck` passed across all 8 packages with the existing TS44 informational JSON messages.
- `bun run release:smoke` passed and refreshed install/lockfile state.
- `bun run build` passed. The build still reports existing Astro `transformWithEsbuild`, tsdown/plugin timing, desktop typeless-module, and large Vite chunk warnings.
- Initial `bun run test` failed in `@luminor/web` on `apps/web/src/appSettings.test.ts` because the persisted-settings decode-default fixture still expected `enableAssistantStreaming: false`; after updating the fixture to the new default, the targeted app settings test passed.
- Final `bun run test` passed: 10 tasks successful in 6m5.217s. `luminor` passed 137 files with 1 skipped file, 1492 passed tests, and 6 skipped tests.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.4`, and `npm run lint` passed.

## 0.3.3 - 2026-06-28

### Added

- Added Windows packaged-app editor discovery so VS Code and VS Code Insiders installed from the Microsoft Store can be launched from Luminor.
...
- Bumped Luminor release package versions to `0.3.3` across the server, desktop, web, and contracts packages.
- Refreshed Luminor icon and logo assets across desktop resources, marketing assets, web favicons, app icons, and shared brand assets.
...
- `bun run lint` passed with 155 warnings, 0 errors.
- Initial `bun run typecheck` failed in `@luminor/web` because `wsNativeApi.test.ts` missed the new `enableProviderUpdateChecks` setting; after that fix it failed in `luminor` because `apps/server/src/open.ts` used unavailable `Effect.catchAll`; after both fixes, `bun run typecheck` passed across all 8 packages with the existing TS44 informational JSON messages.
- `bun run release:smoke` passed and refreshed install/lockfile state.
- `bun run build` passed. The build still reports existing Astro `transformWithEsbuild`, tsdown/plugin timing, desktop typeless-module, and large Vite chunk warnings.
- `bun run test` passed: 10 tasks successful in 5m8.962s. `@luminor/web` passed 188 files / 2212 tests. `luminor` passed 136 files with 1 skipped file, 1475 passed tests, and 6 skipped tests.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.3`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.2` across the server, desktop, web, and contracts packages.
...
- Initial `bun run typecheck` failed in `@luminor/web` on exact optional property handling in `Sidebar.tsx`, `composerDraftStore.ts`, and `chatProjects.ts`; after targeted fixes, `bun run typecheck` passed across all 8 packages with the existing TS44 informational JSON messages.
- `bun run release:smoke` passed. It refreshed install/lockfile state during `bun install`, with no remaining `bun.lock` diff.
- `bun run build` passed. The build still reports existing Astro `transformWithEsbuild`, tsdown/plugin timing, desktop typeless-module, and large Vite chunk warnings.
- `bun run test` passed: 10 tasks successful in 5m44.64s. `@luminor/web` passed 187 files / 2205 tests. `luminor` passed 136 files with 1 skipped file, 1464 passed tests, and 6 skipped tests.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.2`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.3.1` across the server, desktop, web, and contracts packages.
...
- `bun run build` passed. The build still reports existing Astro `transformWithEsbuild`, tsdown/plugin timing, desktop typeless-module, and large Vite chunk warnings.
- `bun run test` passed: 10 tasks successful in 5m6s. `@luminor/web` passed 182 files / 2164 tests. `luminor` passed 135 files with 1 skipped file, 1456 passed tests, and 6 skipped tests.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.1`, and `npm run lint` passed.

## 0.3.0 - 2026-06-24

### Added

- Added first-class Automations as a real Luminor workspace surface, including contracts, persistence, scheduler leases, run tracking, RPC methods, sidebar navigation, list/detail routes, Current/Paused views, inline detail editing, previous-run history, and triage actions.
...
- Added blob-based browser download handling for local image/generated markdown image downloads so failed local-image responses stay inside Luminor instead of navigating the app window to an API error page.
...
- Bumped Luminor release package versions to `0.3.0` across the server, desktop, web, and contracts packages.
...
- `bun run test` passed: 10 tasks successful in 8m53s. `@luminor/web` passed 180 files / 2102 tests. `luminor` passed 135 files with 1 skipped file, 1418 passed tests, and 6 skipped tests. The server suite was long-running but completed cleanly without a teardown stall.
- Website changelog mirror checks passed in `/Users/emanueledipietro/Developer/luminor-website`: `npm run build` prerendered `/changelog/v0.3.0`, and `npm run lint` passed.
...
- Bumped Luminor release package versions to `0.2.41` across the server, desktop, web, and contracts packages.
...
- Root `bun run test` did not complete cleanly in two attempts: both runs reached a green `@luminor/web` suite (169 files / 1954 tests), then stalled in the `apps/server` Vitest tail. The stale duplicate root/Vitest processes were stopped before continuing verification.
...
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.2.41`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- Bumped Luminor release package versions to `0.2.4` across the server, desktop, web, and contracts packages.
...
- `bun run build` passed. Vite still warns about large web chunks and plugin timings; desktop build still reports the existing typeless `tsdown.config.ts` module warning.
- `bun run test` passed: 10 tasks successful, including `@luminor/web` 169 files / 1954 tests and `luminor` 129 files passed / 1 skipped with 1255 passed / 6 skipped.
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.2.4`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- Bumped Luminor release package versions to `0.2.3` across the server, desktop, web, and contracts packages.
...
- `bun run release:smoke` passed and refreshed release install/lockfile state.
- `bun run build` passed. Vite still warns about large web chunks and plugin timings; desktop build still reports the existing typeless `tsdown.config.ts` module warning.
- `bun run test` passed: 10 tasks successful, including `@luminor/web` 168 files / 1949 tests and `luminor` 129 files passed / 1 skipped with 1246 passed / 6 skipped.
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.2.3`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- Final `bun run test` passed: 10 tasks successful, including `@luminor/web` 167 files / 1935 tests, `effect-acp` 3 files / 24 tests, and `luminor` 129 files passed / 1 skipped with 1241 passed / 6 skipped.
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.2.2`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- Added startup turn reconciliation for provider restarts so Luminor can recover unfinished turns from persisted runtime state instead of leaving stale active work behind.
...
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.2.1`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.2.0`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.1.9`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- Reworked provider skill discovery so provider-native skill lists can merge with Luminor's catalog and fall back cleanly when a provider cannot answer.
...
- `bun run release:smoke` passed.
- `bun run build` passed. Vite still warns about large web chunks and plugin timings; desktop build still reports the existing typeless `tsdown.config.ts` module warning.
- First `bun run test` attempt was interrupted by SIGTERM after partial success; no assertion failure was reported before termination, and `@luminor/web:test` had already passed 152 files / 1740 tests.
...
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.1.8`.
- `npm run lint` in `/Users/emanueledipietro/Developer/luminor-website` passed.
...
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.1.7`.
...
- `apps/marketing` has no `test` script.
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website` passed and generated `/changelog/v0.1.6`.
...
- Refreshed README/release messaging and Luminor desktop update flow documentation around the current app positioning.
...
- `npm run build` in `/Users/emanueledipietro/Developer/luminor-website`
