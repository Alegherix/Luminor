...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `luminor` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@luminor/shared/git`, `@luminor/shared/DrainableWorker`) — no barrel index.
