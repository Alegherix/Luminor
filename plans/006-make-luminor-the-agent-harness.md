# Plan 006: Make Luminor the Authoritative Agent Harness
...
An explicit request to start two Luminor threads produced six threads. The
...
`luminor_create_thread`, tried several invalid provider/model combinations, and
...
1. **Model awareness** — every provider is told, through its strongest native
   instruction channel, that Luminor is the hosting harness and that Luminor
   resources are controlled through `luminor_*` tools.
2. **Capability truth** — agents can ask Luminor which providers, models,
...
```text
user asks for exactly two Luminor threads
  -> parent reads luminor_context / luminor_capabilities
  -> parent calls luminor_create_threads once with exactly two entries
  ...
  -> parent calls luminor_wait_for_threads until both runs are terminal
...
- Luminor is the harness; provider-native subagents are implementation details
  inside a provider turn and never substitute for user-requested Luminor
  threads.
- All model-facing host text comes from one versioned
  `LuminorHarnessPolicy`. Provider adapters only choose the delivery mechanism.
- Every state-changing gateway call is bound to a live Luminor provider session
...
- Unsupported provider/model/options fail before `thread.create`, git branch,
  worktree, or provider side effects. Luminor never interprets `Low` as part of
...
- `apps/server/src/agentGateway/Layers/AgentGateway.ts:73-81` owns the only full
  “Luminor is hosting this session / use luminor\_\*” policy, but
...
- `apps/server/src/provider/Layers/ClaudeAdapter.ts:880-885` tells Claude it is
  running inside Luminor, but does not define Luminor-thread versus native
  subagent precedence.
- `apps/server/src/codexAppServerManager.ts:475-486,620-648` sends Codex
  ...
  include the Luminor harness contract.
- Cursor, Grok, and Droid receive MCP transport configuration and a
  Luminor `clientInfo`, but no shared model-facing host policy. See
...
- `apps/server/src/provider/runtimeLayer.ts:54-70` explicitly excludes OpenCode
  ...
  and a clear “Luminor control unavailable” capability instead of pretending
  parity.
...
- A background native agent that retains the same MCP connection can therefore
  mutate Luminor after the visible parent turn has completed.
...
- predecessor naming — Luminor identity must remain consistent;
...
### `luminor_context` (read-only, idempotent)

```ts
type LuminorContextResult = {
  harness: { name: "Luminor"; policyVersion: string };
  caller: {
    threadId: ThreadId;
    turnId: TurnId | null;
...
### `luminor_capabilities` (read-only, idempotent)
...
### `luminor_create_threads` (destructive, idempotent with request id)
...
Keep `luminor_create_thread` as a backwards-compatible single-entry wrapper
...
### `luminor_wait_for_threads` (read-only, idempotent)

```ts
type LuminorWaitForThreadsInput = {
...
- Rebranding or renaming `luminor_*` tools.
- Disabling provider-native subagents globally. They remain available for
  ...
  explicit Luminor-resource requests.
...
- UI redesign. Existing Luminor MCP logo/labels may consume tool annotations,
...
- Branch from the current feature branch with `codex/luminor-harness-control-plane`.
...
### Step 2: Centralize and deliver the Luminor harness policy
...
1. “You are running inside Luminor. Luminor is the host/harness for this
   session.”
2. Use `luminor_*` for Luminor threads, projects, automations, and coordination.
3. Provider-native `spawn_subagent`/Task/collaboration tools do not create
   Luminor threads and must not substitute for an explicit request for Luminor
   threads.
4. Use one `luminor_create_threads` call for plural requests; the array count is
   exact.
5. Resolve canonical models/options through `luminor_capabilities`; do not guess
   slugs or silently change provider/model.
6. Wait with `luminor_wait_for_threads`, then synthesize all results.
...
- **Claude**: append it to the existing Claude system-prompt append. Add MCP
  ...
  list.
- **Cursor/Grok/Droid ACP paths**: include consistent Luminor clientInfo
  ...
  user-visible Luminor message projection.
- **OpenCode/Kilo**: deliver truthful host identity, but advertise Luminor MCP
...
- **Pi**: deliver host identity through its supported prompt/extension path and
  explicitly advertise that Luminor MCP control is unavailable. Do not simulate
  successful Luminor actions.

Keep MCP initialize `instructions` generated from the same policy. Add tool
annotations and make every description begin with its user-facing Luminor
...
- caller thread id;
- a Luminor-generated provider-session key (not an upstream provider id);
- provider kind;
...
- return the same catalog from `luminor_capabilities` that validation uses;
...
Implement `luminor_wait_for_threads` as a read-only long poll over projections:
...
This cap protects Luminor projections from provider fan-out, but it does not
disable native subagent execution. Keep native-child budgeting separate from
the ordinary top-level `luminor_create_threads` operation.
...
- the active parent path uses one two-entry Luminor batch;
...
- [x] Every provider receives the same versioned Luminor host identity through
      its strongest available instruction channel.
- [x] Every provider truthfully reports whether it can call Luminor MCP; no
      provider simulates unsupported host actions.
- [x] MCP initialize identity, tool descriptions, and native host instructions
      all say Luminor, never a predecessor or provider-branded harness.
- [x] Every destructive call requires a live scoped provider session and active
      caller turn.
- [x] A detached background agent cannot create or mutate Luminor threads after
...
- `LuminorHarnessPolicy` is the single source of truth. Any new provider must add
