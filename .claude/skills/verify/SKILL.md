# Verify: run Luminor locally for runtime verification

How to launch an isolated Luminor instance (server + web) to observe UI changes, without touching `~/.luminor` or the default dev ports.

## Launch

```bash
# 1. Server (from the directory you want as the workspace/project cwd):
LUMINOR_HOME=<scratch>/luminor-home \
LUMINOR_PORT=3899 LUMINOR_MODE=web LUMINOR_NO_BROWSER=1 \
