...
- Publishes the CLI package (`apps/server`, npm package `@luminor/cli`) with OIDC trusted publishing.
...
- Required Luminor release assets for updater:
  - platform installers (`.exe`, `.dmg`, `.AppImage`, plus macOS `.zip` for Squirrel.Mac update payloads)
  - `luminor-mac.yml`, `luminor.yml`, and `luminor-linux.yml` metadata
  - every stable release includes both `luminor-mac.yml`, `luminor.yml`, `luminor-linux.yml` and `latest-mac.yml`, `latest.yml`, `latest-linux.yml`
...
- macOS metadata note:
  ...
  - The workflow merges the per-arch macOS metadata, then keeps the merged manifest as `latest-mac.yml` and copies it to `luminor-mac.yml` for stable releases.
...
1. Confirm the npm account controls the `@luminor` scope and can publish `@luminor/cli`.
...
- Optional jobs stay disabled unless repository variables enable them:
  - `LUMINOR_PUBLISH_CLI=1`
  - `LUMINOR_FINALIZE_RELEASE=1`
