...
function buildFingerprint({ arch, release, sources, targets }) {
  ...
  hash.update("luminor-appsnap-helper-build-v1\0");
  hash.update(arch);
  hash.update("\0");
...
export function buildAppSnapHelper({
  ...
  const temporaryDirectory = mkdtempSync(join(tmpdir(), "luminor-appsnap-helper-"));
  ...
  try {
    ...
    for (const target of targets) {
      const thinBinary = join(temporaryDirectory, `luminor-appsnap-helper-${target.arch}`);
    ...
    const unsignedBinary = join(temporaryDirectory, "luminor-appsnap-helper");
