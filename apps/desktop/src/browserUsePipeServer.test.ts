...
import {
  ...
  LUMINOR_BROWSER_HOST_CAPABILITY_FD_ENV,
  LUMINOR_BROWSER_HOST_PIPE_ENV,
  LUMINOR_BROWSER_USE_PIPE_ENV,
  resolveBrowserHostPipeBackendEnv,
  resolveConfiguredBrowserHostPipePath,
  resolveDefaultBrowserHostPipePath,
} from "./browserUsePipeServer";

const TEST_CAPABILITY = "luminor-browser-host-test-capability-0123456789";
...
async function withPipeServer(
  ...
  const directory = await mkdtemp(join(tmpdir(), "luminor-browser-host-test-"));
...
describe("canonical browser host pipe resolution", () => {
  it("creates a private unguessable Unix socket path", () => {
    ...
    expect(dirname(first)).toBe(`/tmp/luminor-browser-host${uidSuffix}`);
  ...
  it("creates an unguessable per-process Windows named pipe", () => {
    ...
    expect(pipePath).toMatch(/^\\\\\.\\pipe\\luminor-browser-host-456-[0-9a-f-]{36}$/);
  ...
  it("prefers the canonical env and accepts the legacy env as fallback", () => {
    expect(
      resolveConfiguredBrowserHostPipePath(
        {
          [LUMINOR_BROWSER_HOST_PIPE_ENV]: "/canonical.sock",
          [LUMINOR_BROWSER_USE_PIPE_ENV]: "/legacy.sock",
        },
        "darwin",
      ),
    ).toBe("/canonical.sock");
    expect(
      resolveConfiguredBrowserHostPipePath(
        {
          [LUMINOR_BROWSER_USE_PIPE_ENV]: String.raw`\\.\pipe\legacy`,
        },
        "win32",
      ),
  ...
  it("publishes both env names only while a listener is active", () => {
    const inherited = {
      KEEP_ME: "yes",
      [LUMINOR_BROWSER_HOST_PIPE_ENV]: "/stale-canonical.sock",
      [LUMINOR_BROWSER_USE_PIPE_ENV]: "/stale-legacy.sock",
    };
    ...
    expect(resolveBrowserHostPipeBackendEnv(inherited, "/active.sock", 3)).toEqual({
      KEEP_ME: "yes",
      [LUMINOR_BROWSER_HOST_PIPE_ENV]: "/active.sock",
      [LUMINOR_BROWSER_USE_PIPE_ENV]: "/active.sock",
      [LUMINOR_BROWSER_HOST_CAPABILITY_FD_ENV]: "3",
    });
  });

  it("rejects a non-private configured parent without changing its permissions", async () => {
    const directory = await mkdtemp(join(tmpdir(), "luminor-browser-host-public-test-"));
