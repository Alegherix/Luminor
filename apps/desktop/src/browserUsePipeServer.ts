...
const PIPE_DIR = "luminor-browser-host";
const PIPE_NAME_PREFIX = "luminor-browser-host";

export const LUMINOR_BROWSER_HOST_PIPE_ENV = "LUMINOR_BROWSER_HOST_PIPE_PATH";
export const LUMINOR_BROWSER_HOST_CAPABILITY_ENV = "LUMINOR_BROWSER_HOST_CAPABILITY";
export const LUMINOR_BROWSER_HOST_CAPABILITY_FD_ENV = "LUMINOR_BROWSER_HOST_CAPABILITY_FD";
/** @deprecated Read/written only while old backend builds are still supported. */
export const LUMINOR_BROWSER_USE_PIPE_ENV = "LUMINOR_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserHostPipePath(
  ...
  const configured =
    env[LUMINOR_BROWSER_HOST_PIPE_ENV]?.trim() || env[LUMINOR_BROWSER_USE_PIPE_ENV]?.trim();
...
export const LUMINOR_BROWSER_HOST_PIPE_PATH = resolveConfiguredBrowserHostPipePath();
/** @deprecated Compatibility alias for old packaged backend builds. */
export const LUMINOR_BROWSER_USE_PIPE_PATH = LUMINOR_BROWSER_HOST_PIPE_PATH;
...
export function resolveBrowserHostPipeBackendEnv(
  ...
  delete backendEnv[LUMINOR_BROWSER_HOST_PIPE_ENV];
  delete backendEnv[LUMINOR_BROWSER_USE_PIPE_ENV];
  delete backendEnv[LUMINOR_BROWSER_HOST_CAPABILITY_ENV];
  delete backendEnv[LUMINOR_BROWSER_HOST_CAPABILITY_FD_ENV];
  const pipePath = activePipePath?.trim();
  if (pipePath && Number.isInteger(capabilityFd) && (capabilityFd ?? 0) >= 3) {
    backendEnv[LUMINOR_BROWSER_HOST_PIPE_ENV] = pipePath;
    backendEnv[LUMINOR_BROWSER_USE_PIPE_ENV] = pipePath;
    backendEnv[LUMINOR_BROWSER_HOST_CAPABILITY_FD_ENV] = String(capabilityFd);
  }
  return backendEnv;
}
...
export class BrowserHostPipeServer {
  ...
  constructor(
    ...
    options: BrowserHostPipeServerOptions | string = LUMINOR_BROWSER_HOST_PIPE_PATH,
  ) {
    const normalized = typeof options === "string" ? { pipePath: options } : options;
    this.platform = normalized.platform ?? process.platform;
    this.pipePath = normalized.pipePath ?? LUMINOR_BROWSER_HOST_PIPE_PATH;
