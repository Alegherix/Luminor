import { ThreadId, type BrowserElementRef, type BrowserSnapshotId } from "@luminor/contracts";
...
const createWebContents = () => {
  ...
  const sendCommand = vi.fn(async (method: string, params?: Record<string, unknown>) => {
    ...
    if (method === "Runtime.evaluate") {
      ...
      if (
        expression.includes('const key = "__luminorBrowserAutomationV1"') &&
      ...
      if (expression.includes("globalThis.__luminorBrowserAutomationV1.currentTarget")) {
...
describe("DesktopBrowserAutomationHost", () => {
  ...
  it("executes the complete 22-tool catalogue against one shared visible runtime", async () => {
    ...
    const workspaceRoot = await mkdtemp(join(tmpdir(), "luminor-browser-host-upload-"));
    const uploadUserDataRoot = await mkdtemp(join(tmpdir(), "luminor-browser-host-user-data-"));
