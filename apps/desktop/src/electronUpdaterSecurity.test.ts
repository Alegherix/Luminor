...
describe("electronUpdaterSecurity", () => {
  ...
  it("parses distinguished names the same way as builder-util-runtime", () => {
    const parsed = parseDistinguishedName('CN=Luminor, O="Acme, Inc.", OU=Tools\\2C Desktop');

    expect(parsed.get("CN")).toBe("Luminor");
  ...
  it("uses only embedded full publisher DNs and never feed-controlled names", () => {
    expect(
      resolveWindowsUpdatePublisherNames(
        ...
        [" CN=Luminor, O=Acme Tools ", "CN=Only", ""],
      ),
    ).toEqual(["CN=Luminor, O=Acme Tools"]);
  ...
  it("validates a matching full distinguished name with shell-free execFile options", async () => {
    const execFile = vi.fn((file, args, options, callback) => {
      callback(
        ...
        JSON.stringify({
          Status: 0,
          Path: "C:\\Users\\test\\AppData\\Local\\Temp\\LuminorSetup.exe",
          SignerCertificate: {
            Subject: "CN=Luminor, O=Acme Tools",
          },
        }),
        "",
      );
    ...
    const result = await verifyWindowsUpdateCodeSignature(
      ["CN=Luminor, O=Acme Tools"],
      "C:\\Users\\test\\AppData\\Local\\Temp\\LuminorSetup.exe",
  ...
  it("rejects a CN-only publisher allowlist", async () => {
      ...
      logger,
      {
        ...
        execFile: vi.fn((_file, _args, _options, callback) => {
          callback(
            ...
            JSON.stringify({
              Status: 0,
              Path: "C:\\Temp\\LuminorSetup.exe",
              SignerCertificate: { Subject: "CN=Luminor, O=Acme Tools" },
            }),
            "",
          );
        }),
    ...
    expect(result).toContain("publisherNames: CN=Luminor");
  ...
  it("returns a mismatch summary for an unexpected publisher", async () => {
    ...
    expect(result).toContain("publisherNames: CN=Luminor, O=Acme Tools");
  ...
  it("patches electron-updater BaseUpdater spawnSyncLog only on Windows", () => {
    ...
    const prototype = FakeBaseUpdater.prototype as {
      ...
      __luminorSpawnSyncLogPatched?: boolean;
    };
    ...
    expect(prototype.__luminorSpawnSyncLogPatched).toBe(true);
  });
