...
export class DesktopAppSnapManager {
  ...
  #handleWatchMessage(child: AppSnapHelperProcess, message: AppSnapHelperMessage): void {
    ...
    // Benign overlap errors surface as a toast without yanking Luminor to the
  ...
  async #consumeCapture(
    ...
    if (discardedRecord) {
      ...
      this.#emitCaptureError(
        ...
        `Luminor could retain only the latest ${MAX_PENDING_CAPTURES} AppSnaps while the composer was unavailable. The oldest capture was discarded.`,
