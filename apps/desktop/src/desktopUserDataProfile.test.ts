...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "luminor-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Luminor profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "luminor-dev" })).toBe(
      "/Users/tester/Library/Application Support/luminor-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "luminor" })).toBe(
      "/Users/tester/Library/Application Support/luminor",
    );
    expect(
      resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "luminor-canary" }),
    ).toBe("/Users/tester/Library/Application Support/luminor-canary");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "luminor");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "luminor-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "luminor-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Luminor profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Luminor database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "luminor");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".luminor-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "luminor");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "luminor-profile-seed.json"), "{");
