...
import { isBrowserCopyLinkChord } from "@luminor/shared/browserShortcuts";
import {
...
export class DesktopBrowserManager {
  ...
  private scheduleWindowOpenTab(input: {
    ...
    // runtimes. Only the first decision may create a canonical Luminor tab.
    if (
  ...
  attachWebview(input: BrowserAttachWebviewInput, hostWebContentsId: number): ThreadBrowserState {
    ...
    if (
      ...
      throw new Error("The browser webview does not belong to this Luminor window and partition.");
    }
