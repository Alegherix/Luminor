    ...
    if application.bundleIdentifier == excludedBundleIdentifier {
        return .failure(
            AppSnapFailure(
                ...
                message: "Luminor cannot capture its own window."
            )
        )
    }
...
final class OneFrameWindowCapture: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    private let outputQueue = DispatchQueue(label: "dev.luminor.appsnap.stream-output")
...
final class AppSnapCaptureCoordinator {
    ...
    private let queue = DispatchQueue(label: "dev.luminor.appsnap.capture")
