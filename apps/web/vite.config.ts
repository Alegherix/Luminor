// FILE: vite.config.ts
// Purpose: Builds the Luminor web client and controls diagnostic source maps.
...
const sourcemapEnv = process.env.LUMINOR_WEB_SOURCEMAP?.trim().toLowerCase();
...
function centralIconPrunePlugin(): Plugin {
  ...
  return {
    name: "luminor-central-icon-prune",
    apply: "build",
