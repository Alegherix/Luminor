...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.luminor/worktrees/luminor-mvp/luminor-4e609bb8",
    );
    expect(result).toBe("luminor-4e609bb8");
  });

  it("keeps legacy .luminor worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.luminor/worktrees/luminor-mvp/luminor-legacy123",
    );
    expect(result).toBe("luminor-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.luminor\\worktrees\\luminor-mvp\\luminor-4e609bb8",
    );
    expect(result).toBe("luminor-4e609bb8");
  });

  it("uses the final segment even when outside ~/.luminor/worktrees", () => {
