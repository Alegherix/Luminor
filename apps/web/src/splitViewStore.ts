...
import { type ProjectId, type ThreadId, type TurnId } from "@luminor/contracts";
...
// Keep the v1 suffix stable while using the Luminor namespace; legacy
// `luminor:*` and `luminor:*` keys are copied over by
// `storageKeyMigration` before this store hydrates, so older payloads still
// flow through the v1 -> v2 schema migration below.
const SPLIT_VIEW_STORAGE_KEY = "luminor:split-view-state:v1";
