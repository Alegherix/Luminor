// FILE: LuminorThreadCreationCard.tsx
// Purpose: End-of-turn recap for threads created through the Luminor MCP harness.
// Layer: Chat transcript UI

import { PROVIDER_DISPLAY_NAMES } from "@luminor/contracts";
import { formatModelDisplayName } from "@luminor/shared/model";
import { memo } from "react";

import type { WorkLogLuminorThreadCreation } from "../../session-logic";
import { ProviderIcon } from "../ProviderIcon";
import { LuminorLogo } from "../LuminorLogo";
import { Button } from "../ui/button";

function threadMeta(thread: WorkLogLuminorThreadCreation["threads"][number]): string {
...
export const LuminorThreadCreationCard = memo(function LuminorThreadCreationCard({
  creation,
  onOpenThread,
}: {
  readonly creation: WorkLogLuminorThreadCreation;
  ...
  return (
    <div
      ...
      data-luminor-thread-creation-card="true"
    >
      <div className="flex min-w-0 items-center gap-3 px-3 py-2.5">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[var(--color-background-elevated-secondary)] text-foreground">
          <LuminorLogo className="h-[22px] w-auto" aria-label="Luminor" />
        </span>
