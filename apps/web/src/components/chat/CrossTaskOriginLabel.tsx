...
// Purpose: Identify the source thread for conversations created by another Luminor agent.
// Layer: Chat transcript UI

import { type ProviderKind, type ThreadId } from "@luminor/contracts";
...
// Luminor thread, so it always reads as "Sent by Luminor" with the Luminor mark
// (the origin provider is not surfaced here to keep one consistent label).
function OriginContent(): ReactNode {
  return (
    <>
      <span className="flex size-4 shrink-0 items-center justify-center text-muted-foreground/70">
        <LuminorLogo className="h-4 w-auto" aria-label="Luminor" />
      </span>
      <span className="truncate">Sent by Luminor from another thread</span>
    </>
  );
}
