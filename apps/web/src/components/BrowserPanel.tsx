...
} from "@luminor/shared/browserSession";
import {
  BROWSER_COPY_LINK_TOAST_TITLE,
  isBrowserCopyLinkChord,
} from "@luminor/shared/browserShortcuts";
...
const BROWSER_WEBVIEW_PARTITION = "persist:luminor-browser";
...
function isBrowserPerfLoggingEnabled(): boolean {
  ...
  try {
    return window.localStorage.getItem("luminor:browser-perf") === "1";
  } catch {
    return false;
  }
}
...
export function BrowserPanel({
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      console.info(`[${LUMINOR_BROWSER_LABEL} panel perf]`, {
        threadId,
