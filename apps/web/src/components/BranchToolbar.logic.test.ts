import type { GitBranch } from "@luminor/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.luminor/worktrees/feature-a",
  ...
  it("keeps legacy .luminor worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.luminor/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/luminor-pi",
        existingAssociatedWorktreeBranch: "luminor/pi",
        existingAssociatedWorktreeRef: "luminor/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/luminor-pi",
      associatedWorktreeBranch: "luminor/pi",
      associatedWorktreeRef: "luminor/pi",
    });
  });
  ...
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        branch: {
          isDefault: false,
          worktreePath: "/repo/.luminor/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.luminor/worktrees/feature-b",
      nextWorktreePath: "/repo/.luminor/worktrees/feature-b",
