...
import { describeErrorMessage } from "@luminor/shared/errorMessages";
import {
  TERMINAL_MAX_COLS,
  TERMINAL_MAX_ROWS,
  TERMINAL_MIN_COLS,
...
import type { TerminalSessionSnapshot } from "@luminor/contracts";
...
const TERMINAL_PARKING_CONTAINER_ID = "luminor-terminal-parking";

type LuminorTerminalOptions = NonNullable<ConstructorParameters<typeof Terminal>[0]> & {
...
const TERMINAL_CURSOR_STYLE: NonNullable<LuminorTerminalOptions["cursorStyle"]> = "bar";
const TERMINAL_INACTIVE_CURSOR_STYLE: NonNullable<LuminorTerminalOptions["cursorInactiveStyle"]> =
  "bar";
...
function syncTheme(entry: TerminalRuntimeEntry): void {
  ...
  const terminalOptions = entry.terminal.options as LuminorTerminalOptions;
...
export function createRuntimeEntry(config: TerminalRuntimeConfig): TerminalRuntimeEntry {
  ...
  const terminalOptions: LuminorTerminalOptions = {
    cursorBlink: true,
