...
import type { DesktopUpdateActionResult, DesktopUpdateState } from "@luminor/contracts";
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  const action = resolveDesktopUpdateButtonAction(state);
  if (action === "download") {
    return "This Mac has Apple Silicon, but Luminor is still running the Intel build under Rosetta. Luminor is preparing the native Apple Silicon update.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but Luminor is still running the Intel build under Rosetta. Click Update to restart into the native Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but Luminor is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
...
export function getDesktopUpdateButtonTooltip(
  ...
  if (state.errorContext === "install" && !state.downloadedVersion && state.availableVersion) {
    return `Luminor restarted, but update ${state.availableVersion} was not installed. Click to try again.`;
  }
