...
import { appSnapShortcutLabels } from "@luminor/shared/appSnapShortcut";
...
function appSnapStatusText(state: DesktopAppSnapState | null): string {
  if (!state) return "Available in the Luminor desktop app";
...
export function NotificationsSettingsPanel({
  ...
  async function sendTestNotification() {
    ...
    const notification = new Notification(title, { body, tag: "luminor:test-notification" });
  ...
  async function setAppSnapEnabled(nextEnabled: boolean) {
    ...
    if (!bridge) {
      toastManager.add({
        ...
        description: "AppSnap requires the Luminor desktop app on macOS.",
      });
      return;
    }
    ...
    <div className="space-y-6">
      <SettingsCard divided={false} className="flex items-start gap-3 px-4 py-3.5">
        ...
        <div className="min-w-0 space-y-1">
          ...
          <p className={SETTINGS_CARD_ROW_DESCRIPTION_CLASS_NAME}>
            Press your two-key shortcut while any app is frontmost. Luminor captures that window as
          ...
          {!supported ? (
            <p className={cn(SETTINGS_CARD_ROW_DESCRIPTION_CLASS_NAME, "pt-0.5")}>
              {appSnapState
                ...
                : "AppSnap requires the Luminor desktop app on macOS."}
            </p>
          ) : null}
        </div>
      </SettingsCard>
      ...
      <SettingsSection title="Capture">
        <SettingsRow
          ...
          description="Run the capture listener in the background while Luminor is open."
        ...
        <SettingsRow
          title="Shortcut"
          description="Choose exactly two keys: one modifier and one other key. Luminor checks its own bindings and asks macOS whether another app already owns the shortcut before saving it."
          control={
        ...
        <SettingsRow
          title="Destination"
          description="Snaps join the task you interacted with in the last minute, and consecutive snaps stay together. Otherwise Luminor opens a fresh task with the capture attached."
      ...
      {supported ? (
        <SettingsSection title="macOS permissions">
          <SettingsRow
            ...
            description="Lets Luminor notice the double-Option chord while another app owns the keyboard. Nothing you type is recorded."
            control={<AppSnapPermissionBadge permission={appSnapState.inputMonitoringPermission} />}
          />
          <SettingsRow
            title="Screen Recording"
            description="Lets Luminor capture an image of the frontmost window. Only the single window you snap is captured, only at the moment you press the chord."
            control={<AppSnapPermissionBadge permission={appSnapState.screenRecordingPermission} />}
          />
          <SettingsRow
            title="Permission status"
            description="Grant both permissions to Luminor under System Settings → Privacy & Security, then recheck here. macOS may require relaunching the app after a change."
            control={
              <Button
                type="button"
                size="xs"
