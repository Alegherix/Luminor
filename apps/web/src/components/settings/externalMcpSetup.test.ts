...
describe("external MCP guided setup", () => {
  it("builds copyable Codex and Claude Code commands without embedding a credential", () => {
    ...
    expect(codex.value).toBe(
      "codex mcp add luminor --env ELECTRON_RUN_AS_NODE=1 -- /Applications/Luminor.app/Contents/MacOS/Luminor server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Luminor home'",
    );
    expect(claude.value).toBe(
      "claude mcp add --scope user luminor -e ELECTRON_RUN_AS_NODE=1 -- /Applications/Luminor.app/Contents/MacOS/Luminor server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Luminor home'",
    );
  ...
  it("builds standard JSON configuration for desktop and other clients", () => {
    ...
    const parsed = JSON.parse(desktop.value) as {
      mcpServers: { luminor: { command: string; args: ReadonlyArray<string> } };
    };

    expect(desktop.format).toBe("json");
    expect(parsed.mcpServers.luminor).toEqual(stdio);
  });
  ...
  it("builds a project-specific prompt without exposing implementation identifiers", () => {
    const prompt = buildExternalMcpExamplePrompt("Luminor app");

    expect(prompt).toContain('project named "Luminor app"');
  ...
  it("builds one agent-facing setup prompt covering pairing, registration, and verification", () => {
    const prompt = buildExternalMcpSetupPrompt({
      setupCommand: "luminor mcp pair --code syn_pair_v1_example --home-dir /tmp/home",
      stdio,
    });

    expect(prompt).toContain("syn_pair_v1_example");
    expect(prompt).toContain("codex mcp add luminor");
    expect(prompt).toContain("claude mcp add --scope user luminor");
    expect(prompt).toContain('"mcpServers"');
    expect(prompt).toContain("luminor_overview");
