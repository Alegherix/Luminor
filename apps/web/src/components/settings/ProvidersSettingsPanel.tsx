              ...
              {showProviderUpdateStatus && updateAdvisory?.status === "behind_latest" ? (
                <div className="text-xs text-muted-foreground">
                  {updateAdvisory.canUpdate && updateAdvisory.updateCommand ? (
                    ...
                    "A newer version is available, but Luminor could not identify a safe one-click update command for this installation."
                  )}
                </div>
              ) : null}
              {showSelfManagedUpdate && props.providerStatus ? (
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0 text-xs text-muted-foreground">
                    {title} manages its own releases, so Luminor cannot tell whether a newer version
      ...
      <div id={SETTINGS_TARGETS.providerUpdates}>
        <SettingsSection title="Updates">
          ...
          <SettingsRow
            ...
            description="Review installed provider tools that Luminor can safely update."
            status={
