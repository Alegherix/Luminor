...
import type { WsCompatibilityError } from "@luminor/contracts";

export type WsTransportState = "connecting" | "open" | "closed" | "incompatible" | "disposed";

export const LUMINOR_WS_TRANSPORT_STATE_EVENT = "luminor:ws-transport-state";
export const LUMINOR_WS_COMPATIBILITY_ISSUE_EVENT = "luminor:ws-compatibility-issue";
...
export function emitWsTransportState(state: WsTransportState): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsTransportStateEventDetail>(LUMINOR_WS_TRANSPORT_STATE_EVENT, {
      detail: { state },
    }),
  );
}
...
export function addWsTransportStateListener(
  ...
  window.addEventListener(LUMINOR_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  return () => {
    window.removeEventListener(LUMINOR_WS_TRANSPORT_STATE_EVENT, handleStateChange);
  };
}
...
export function emitWsCompatibilityIssue(issue: WsCompatibilityError | null): void {
  ...
  window.dispatchEvent(
    new CustomEvent<WsCompatibilityIssueEventDetail>(LUMINOR_WS_COMPATIBILITY_ISSUE_EVENT, {
      detail: { issue },
    }),
  );
}
...
export function addWsCompatibilityIssueListener(
  ...
  window.addEventListener(LUMINOR_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  return () => {
    window.removeEventListener(LUMINOR_WS_COMPATIBILITY_ISSUE_EVENT, handleIssue);
  };
}
