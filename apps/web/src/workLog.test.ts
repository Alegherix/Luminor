import { MessageId, TurnId, type OrchestrationThreadActivity } from "@luminor/contracts";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("keeps created-automation milestones and exposes their card fields despite a null turn id", () => {
    const activities: OrchestrationThreadActivity[] = [
      ...
      makeActivity({
        ...
        summary: "Created automation: Watch Luminor PR 231 - Every 5m",
        tone: "info",
        payload: {
          source: "chat-composer",
          automationId: "automation-7",
          automationName: "Watch Luminor PR 231",
  ...
  it("exposes a provider-independent Luminor thread creation recap", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "luminor-created-threads",
        createdAt: "2026-02-23T00:00:05.000Z",
        turnId: "turn-1",
        kind: "luminor.threads.created",
        summary: "Created 2 Luminor threads",
        tone: "info",
        payload: {
    ...
    expect(entry?.luminorThreadCreation).toEqual({
      operationId: "gateway:create:two-workers",
      requestedCount: 2,
      createdCount: 2,
      threads: [
  ...
  it("hides repeated non-adjacent recovery rows for the same provider turn", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        summary: "Luminor recovered a stale running state",
        turnId: "turn-stale",
              ...
              commandActions: [
                {
                  ...
                  name: "README.md",
                  path: "/Users/emanueledipietro/Developer/Testing/luminor/README.md",
                },
              ],
            },
          },
        ...
        payload: {
          ...
          data: {
            item: {
              ...
              command: "/bin/zsh -lc 'git status --short'",
              cwd: "/Users/emanueledipietro/Developer/Testing/luminor",
              status: "completed",
              commandActions: [{ type: "unknown", command: "git status --short" }],
  ...
  it("preserves cancellation when an owning turn aborts", () => {
    const turnId = TurnId.makeUnsafe("turn-with-cancelled-luminor-tool");
    const entries = deriveWorkLogEntries(
      [
        makeActivity({
          id: "cancelled-luminor-start",
          createdAt: "2026-02-23T00:00:01.000Z",
          kind: "tool.started",
          summary: "Luminor create thread",
          turnId,
          payload: {
            itemType: "mcp_tool_call",
            title: "Luminor create thread",
            data: {
              toolCallId: "cancelled-luminor-call",
      ...
      [
        makeActivity({
          ...
          payload: {
            ...
            status: "interrupted",
            data: {
              toolCallId: "interrupted-luminor-call",
              toolName: "mcp__luminor__luminor_create_thread",
            },
          },
        }),
  ...
  it("presents Luminor MCP activity consistently across provider item shapes", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "luminor-mcp-create-thread-progress",
        kind: "tool.updated",
        ...
        payload: {
          ...
          title: "MCP tool call",
          data: {
            toolCallId: "luminor-mcp-create",
            toolName: "mcp__luminor__luminor_create_thread",
          },
        },
      }),
      makeActivity({
        id: "luminor-dynamic-send-message-progress",
        kind: "tool.updated",
        summary: "Tool call",
        payload: {
          itemType: "dynamic_tool_call",
          title: "Luminor__luminor_send_message",
          data: {
            toolCallId: "luminor-dynamic-send",
          },
        },
      }),
      makeActivity({
        id: "luminor-file-change-list-threads-progress",
        kind: "tool.updated",
        summary: "File change",
        payload: {
          itemType: "file_change",
          title: "mcp__Luminor__luminor_list_threads",
          data: {
            toolCallId: "luminor-file-change-list",
          },
        },
      }),
    ];
    ...
    expect(entries.map((entry) => [entry.itemType, entry.toolTitle])).toEqual(
      expect.arrayContaining([
        ["mcp_tool_call", "Luminor is creating a thread"],
        ["dynamic_tool_call", "Luminor is sending a message"],
        ["file_change", "Luminor is listing threads"],
      ]),
    );
    expect(entries).toHaveLength(3);
  });

  it("preserves a failed Luminor MCP result as a failed activity sentence", () => {
    const [entry] = deriveWorkLogEntries(
      [
        makeActivity({
          id: "luminor-create-threads-failed",
          kind: "tool.completed",
          summary: "luminor__luminor_create_threads",
          payload: {
            itemType: "mcp_tool_call",
            status: "failed",
            data: {
              toolCallId: "luminor-create-failed",
    ...
    expect(entry).toMatchObject({
      toolStatus: "failed",
      toolTitle: "Luminor couldn't create threads",
