...
// Purpose: Imports Luminor browser state before renderer stores hydrate after a desktop origin move.
...
export function importLuminorStorageSnapshot(
  snapshot: LuminorStorageSnapshot | null,
...
export function bootstrapLuminorStorageOriginMigration(): void {
  const bridge = globalThis.window?.desktopBridge?.storageMigration;
  if (!bridge) return;

  try {
    const snapshot = bridge.readSnapshot();
    if (snapshot && importLuminorStorageSnapshot(snapshot)) {
...
bootstrapLuminorStorageOriginMigration();
