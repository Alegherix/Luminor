...
} from "@luminor/shared/composerSlashCommands";
...
function shouldKeepBuiltInSlashCommandDespiteNativeCollision(
  ...
  return (
    ...
    command === "export" ||
    command === "feedback" ||
    // /fork is app-owned everywhere: it creates a Luminor thread with fork
...
> = {
  ...
  feedback: {
    command: "feedback",
    label: "/feedback",
    description: "Send feedback to the Luminor team",
    source: "app",
  },
  automation: {
    command: "automation",
  ...
  const availableCommands: ComposerSlashCommand[] =
    input.provider !== "claudeAgent"
      ...
      : [
          ...
          // creates a Luminor split/context clone before the provider sees the first turn.
          // /fork is app-level for the same reason — it creates a Luminor thread with fork
          // lineage (native session forking under the hood), not a provider text command.
          // /export is app-level too — Luminor owns the thread transcript, so the download
