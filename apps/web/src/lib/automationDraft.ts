...
import { DEFAULT_AUTOMATION_FAST_INTERVAL_MAX_ITERATIONS } from "@luminor/contracts";
import type {
  AutomationMode,
  AutomationSchedule,
...
export function buildAutomationDraftWarnings(input: {
    ...
    warnings.push({
      ...
      detail:
        input.worktreeMode === "auto"
          ? "If Luminor cannot create a worktree, runs may fall back to editing the active project checkout."
  ...
  if (
    ...
    warnings.push({
      ...
      detail: "Luminor was not fully confident about the parsed automation fields.",
      requiresAcknowledgement: false,
    });
  }
