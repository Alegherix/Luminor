...
import { isSupportedLocalPreviewFilePath } from "@luminor/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
  isWorkspaceRelativePathSafe,
  workspaceRelativePathOf,
} from "@luminor/shared/path";
import { isScratchWorkspacePath } from "@luminor/shared/threadWorkspace";
...
const LUMINOR_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const LUMINOR_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveLuminorPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!LUMINOR_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${LUMINOR_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveLuminorPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
