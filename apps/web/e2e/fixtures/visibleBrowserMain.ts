...
import type { BrowserAnnotationEvent, ThreadBrowserState, ThreadId } from "@luminor/contracts";
...
const pipePath = process.env.LUMINOR_BROWSER_HOST_PIPE_PATH;
const capability = process.env.LUMINOR_BROWSER_HOST_CAPABILITY;
const shellPath = process.env.LUMINOR_E2E_SHELL_PATH;
const threadId = process.env.LUMINOR_E2E_THREAD_ID as ThreadId | undefined;
const luminorHome = process.env.LUMINOR_HOME;
const annotationPreloadPath = process.env.LUMINOR_E2E_BROWSER_ANNOTATION_PRELOAD;

if (!pipePath || !capability || !shellPath || !threadId || !luminorHome || !annotationPreloadPath) {
  throw new Error("The visible-browser Electron fixture requires its isolated E2E environment.");
}

app.setPath("userData", path.join(luminorHome, "electron-userdata"));
...
function setPanelVisible(visible: boolean): void {
  ...
  pushState();
  mainWindow?.webContents.send("luminor-e2e:open-panel");
}
function pushState(): void {
  if (shellReady && latestState && mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("luminor-e2e:browser-state", latestState);
  }
}
...
ipcMain.on("luminor-e2e:shell-ready", () => {
  shellReady = true;
  pushState();
});

ipcMain.handle(
  "luminor-e2e:attach-webview",
