...
import type { BrowserAnnotationEvent, BrowserAnnotationTheme } from "@luminor/contracts";
...
test("a real Electron guest commits and reprojects a continuous annotation session", async () => {
  const mainPath = process.env.LUMINOR_E2E_ELECTRON_MAIN;
  ...
  const home = mkdtempSync(join(tmpdir(), "luminor-browser-annotations-e2e-"));
  ...
  const electronApp = await electron.launch({
    ...
    env: {
      ...
      HOME: home,
      LUMINOR_HOME: home,
      LUMINOR_BROWSER_HOST_PIPE_PATH: pipePath,
      LUMINOR_BROWSER_HOST_CAPABILITY: capability,
      LUMINOR_E2E_SHELL_PATH: shellPath,
      LUMINOR_E2E_THREAD_ID: threadId,
      LUMINOR_E2E_BROWSER_ANNOTATION_PRELOAD: annotationPreloadPath,
    },
  });
  ...
  try {
    ...
    const sendNativeInput = (event: Record<string, unknown>) =>
      electronApp.evaluate(
        (_electron, input) => {
          ...
          ).__luminorVisibleBrowserE2E.browserManager;
    ...
    const annotationEvents = async (): Promise<BrowserAnnotationEvent[]> =>
      electronApp.evaluate(() => {
        const fixture = (
          globalThis as typeof globalThis & {
            __luminorVisibleBrowserE2E: { annotationEvents: BrowserAnnotationEvent[] };
          }
        ).__luminorVisibleBrowserE2E;
