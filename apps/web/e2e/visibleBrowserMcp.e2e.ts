...
import { BROWSER_TOOL_NAMES } from "@luminor/contracts";
...
test("production MCP controls one persistent Electron page across visibility changes", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "luminor-visible-browser-e2e-"));
  ...
  try {
    ...
    await electronApp.evaluate(() => {
      (
        globalThis as typeof globalThis & {
          __luminorVisibleBrowserE2E: { setPanelRevealEnabled(enabled: boolean): void };
        }
      ).__luminorVisibleBrowserE2E.setPanelRevealEnabled(false);
    });
    ...
    await electronApp.evaluate(() => {
      ...
      ).__luminorVisibleBrowserE2E.setPanelRevealEnabled(true);
    });
    ...
    const runtimeCount = await electronApp.evaluate(() => {
      const manager = (
        globalThis as typeof globalThis & {
          __luminorVisibleBrowserE2E: { browserManager: { runtimes: Map<string, unknown> } };
        }
