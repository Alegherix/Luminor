...
import { AuthSessionId } from "@luminor/contracts";
import {
  ATTACHMENT_CANCEL_ROUTE_PATH,
  ATTACHMENT_UPLOAD_ROUTE_PATH,
} from "@luminor/shared/binaryTransfer";
...
function makeServerAuth(sideEffects: { count: number }): ServerAuthShape {
  ...
  const descriptor = {
    ...
    sessionCookieName: "luminor_session",
  };
  ...
  return {
    ...
    authenticateHttpRequest: (request) => {
      ...
      const cookie = request.cookies.luminor_session === "cookie-token";
      if (!bearer && !cookie) {
    ...
    issueStartupPairingUrl: () =>
      Effect.succeed("https://luminor.example.test/pair#token=PAIRINGTOKEN"),
  ...
  return {
    ...
    headers: {
      ...
      ...(input.credential === "bearer"
        ...
        : { Cookie: "luminor_session=cookie-token" }),
...
describe("authEffectRouteLayer", () => {
  ...
  it("logs out either role and clears the exact cookie with secure public-mode attributes", async () => {
    ...
    await withAuthEffectServer(config, makeServerAuth(sideEffects), async (serverOrigin) => {
      ...
      expect(cookie).toContain("luminor_session=");
...
describe("binaryUploadEffectRouteLayer", () => {
  it("allows credentialed Canary attachment upload preflights", async () => {
    const config = {
      host: "127.0.0.1",
      attachmentsDir: fs.mkdtempSync(path.join(os.tmpdir(), "luminor-upload-cors-")),
    } as ServerConfigShape;
    try {
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          expect(response.headers.get("access-control-allow-origin")).toBe("luminor-canary://app");
  ...
  it("rejects ambient cookie uploads without an origin and accepts explicit bearer auth", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-upload-route-"));
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://luminor.example.test/"),
      attachmentsDir,
      ...
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          const cookieResponse = await fetch(url, {
            method: "POST",
            headers: { Cookie: "luminor_session=cookie-token" },
