...
import {
  ...
  LUMINOR_TERMINAL_HOOK_OSC_PREFIX,
  LUMINOR_TERMINAL_CLI_KIND_ENV_KEY,
...
function buildHookOscSequence(eventType: TerminalAgentHookEventType): string {
  return `\\033]${LUMINOR_TERMINAL_HOOK_OSC_PREFIX}${eventType}\\007`;
}
...
set -eu
if [ "$#" -gt 0 ]; then
  _luminor_hook_input="$1"
else
  _luminor_hook_input="$(cat)"
fi

_luminor_extract_event() {
  printf '%s' "$_luminor_hook_input" | sed -n "s/.*\\\"$1\\\"[[:space:]]*:[[:space:]]*\\\"\\([^\\\"]*\\)\\\".*/\\1/p" | head -n 1
}

_luminor_event="$(_luminor_extract_event hook_event_name)"
if [ -z "$_luminor_event" ]; then
  _luminor_type="$(_luminor_extract_event type)"
  case "$_luminor_type" in
    task_started|userPromptSubmitted|user_prompt_submit)
      _luminor_event="Start"
      ;;
    ...
    exec_approval_request|apply_patch_approval_request|request_user_input)
      _luminor_event="PermissionRequest"
      ;;
  esac
fi
...
_luminor_emit_osc() {
  _luminor_sequence="$1"
  if [ -w /dev/tty ]; then
    printf '%b' "$_luminor_sequence" > /dev/tty 2>/dev/null || printf '%b' "$_luminor_sequence"
    return
  fi
  printf '%b' "$_luminor_sequence"
}

case "$_luminor_event" in
  UserPromptSubmit|PostToolUse|PostToolUseFailure|Start)
    _luminor_emit_osc '${buildHookOscSequence("Start")}'
    ;;
  Stop)
    _luminor_emit_osc '${buildHookOscSequence("Stop")}'
    ;;
  PermissionRequest|PreToolUse|Notification)
    _luminor_emit_osc '${buildHookOscSequence("PermissionRequest")}'
    ;;
esac
`;
}
...
function buildCodexWrapperScript(input: {
  ...
  return [
    ...
    '    _luminor_codex_ts="$(date +%s 2>/dev/null || echo "$$")"',
    '    export CODEX_TUI_SESSION_LOG_PATH="${TMPDIR:-/tmp}/luminor-codex-session-$$_${_luminor_codex_ts}.jsonl"',
    "  fi",
    "  (",
    '    _luminor_log="$CODEX_TUI_SESSION_LOG_PATH"',
    `    _luminor_notify=${shellQuote(notifyHookPath)}`,
    '    _luminor_last_turn_id=""',
    '    _luminor_last_approval_id=""',
    '    _luminor_last_exec_call_id=""',
    "    _luminor_approval_fallback_seq=0",
    "",
    "    _luminor_emit_event() {",
    '      _luminor_event="$1"',
    `      _luminor_payload=$(printf '{"hook_event_name":"%s"}' "$_luminor_event")`,
    '      "$_luminor_notify" "$_luminor_payload" >/dev/null 2>&1 || true',
    "    }",
    "",
    "    _luminor_i=0",
    '    while [ ! -f "$_luminor_log" ] && [ "$_luminor_i" -lt 200 ]; do',
    "      _luminor_i=$((_luminor_i + 1))",
    "      sleep 0.05",
    "    done",
    '    if [ ! -f "$_luminor_log" ]; then',
    "      exit 0",
    "    fi",
    "",
    '    tail -n 0 -F "$_luminor_log" 2>/dev/null | while IFS= read -r _luminor_line; do',
    '      case "$_luminor_line" in',
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"task_started"'*)`,
    `          _luminor_turn_id=$(printf '%s\n' "$_luminor_line" | awk -F'"turn_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          [ -n "$_luminor_turn_id" ] || _luminor_turn_id="task_started"',
    '          if [ "$_luminor_turn_id" != "$_luminor_last_turn_id" ]; then',
    '            _luminor_last_turn_id="$_luminor_turn_id"',
    '            _luminor_emit_event "Start"',
    "          fi",
    "          ;;",
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"'*'_approval_request"'*)`,
    `          _luminor_approval_id=$(printf '%s\n' "$_luminor_line" | awk -F'"id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    `          [ -n "$_luminor_approval_id" ] || _luminor_approval_id=$(printf '%s\n' "$_luminor_line" | awk -F'"approval_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    `          [ -n "$_luminor_approval_id" ] || _luminor_approval_id=$(printf '%s\n' "$_luminor_line" | awk -F'"call_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          if [ -z "$_luminor_approval_id" ]; then',
    "            _luminor_approval_fallback_seq=$((_luminor_approval_fallback_seq + 1))",
    '            _luminor_approval_id="approval_request_${_luminor_approval_fallback_seq}"',
    "          fi",
    '          if [ "$_luminor_approval_id" != "$_luminor_last_approval_id" ]; then',
    '            _luminor_last_approval_id="$_luminor_approval_id"',
    '            _luminor_emit_event "PermissionRequest"',
    "          fi",
    "          ;;",
    `        *'"dir":"to_tui"'*'"kind":"codex_event"'*'"msg":{"type":"exec_command_begin"'*)`,
    `          _luminor_exec_call_id=$(printf '%s\n' "$_luminor_line" | awk -F'"call_id":"' 'NF > 1 { sub(/".*/, "", $2); print $2; exit }')`,
    '          if [ -n "$_luminor_exec_call_id" ]; then',
    '            if [ "$_luminor_exec_call_id" != "$_luminor_last_exec_call_id" ]; then',
    '              _luminor_last_exec_call_id="$_luminor_exec_call_id"',
    '              _luminor_emit_event "Start"',
    "            fi",
    "          else",
    '            _luminor_emit_event "Start"',
    ...
    "    done",
    "  ) &",
    "  LUMINOR_CODEX_START_WATCHER_PID=$!",
    "fi",
    `${shellQuote(targetPath)} --enable codex_hooks -c ${shellQuote(`notify=["bash",${JSON.stringify(notifyHookPath)}]`)} "$@"`,
    "_luminor_status=$?",
    'if [ -n "${LUMINOR_CODEX_START_WATCHER_PID:-}" ]; then',
    '  kill "$LUMINOR_CODEX_START_WATCHER_PID" >/dev/null 2>&1 || true',
    '  wait "$LUMINOR_CODEX_START_WATCHER_PID" 2>/dev/null || true',
    "fi",
    'exit "$_luminor_status"',
  ].join("\n");
}
...
function buildWrapperScript(input: {
  ...
  return [
    "#!/bin/sh",
    `# Managed ${commandName} wrapper injected by luminor terminal sessions.`,
    `printf '\\033]0;%s\\007' ${shellQuote(title)}`,
    `export ${LUMINOR_TERMINAL_CLI_KIND_ENV_KEY}=${shellQuote(cliKind)}`,
    commandBody,
    "",
  ].join("\n");
}
...
function buildManagedZshRc(quotedZshDir: string): string {
  return `# Luminor zsh rc wrapper
_luminor_home="\${LUMINOR_ORIGINAL_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_luminor_home"
[[ -f "$_luminor_home/.zshrc" ]] && source "$_luminor_home/.zshrc"
export ZDOTDIR=${quotedZshDir}
if [ -n "\${LUMINOR_MANAGED_BIN_DIR:-}" ] && [ -d "\${LUMINOR_MANAGED_BIN_DIR}" ]; then
  case ":$PATH:" in
    *:\${LUMINOR_MANAGED_BIN_DIR}:*) ;;
    *) export PATH="\${LUMINOR_MANAGED_BIN_DIR}:$PATH" ;;
  esac
  unalias claude 2>/dev/null || true
  claude() {
    if [ -x "\${LUMINOR_MANAGED_BIN_DIR}/claude" ] && [ ! -d "\${LUMINOR_MANAGED_BIN_DIR}/claude" ]; then
      "\${LUMINOR_MANAGED_BIN_DIR}/claude" "$@"
    else
      command claude "$@"
    fi
  }
  ...
  codex() {
    if [ -x "\${LUMINOR_MANAGED_BIN_DIR}/codex" ] && [ ! -d "\${LUMINOR_MANAGED_BIN_DIR}/codex" ]; then
      "\${LUMINOR_MANAGED_BIN_DIR}/codex" "$@"
    else
      command codex "$@"
    fi
  }
  typeset -ga precmd_functions 2>/dev/null || true
  _luminor_ensure_managed_bin() {
    case ":$PATH:" in
      *:\${LUMINOR_MANAGED_BIN_DIR}:*) ;;
      *) PATH="\${LUMINOR_MANAGED_BIN_DIR}:$PATH" ;;
    esac
  }
  {
    precmd_functions=(\${precmd_functions:#_luminor_ensure_managed_bin} _luminor_ensure_managed_bin)
  } 2>/dev/null || true
fi
`;
}
...
function ensureManagedZshWrappers(zshDir: string): void {
  ...
  writeFileIfChanged(
    ...
    `# Luminor zsh env wrapper
_luminor_home="\${LUMINOR_ORIGINAL_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_luminor_home"
[[ -f "$_luminor_home/.zshenv" ]] && source "$_luminor_home/.zshenv"
export ZDOTDIR=${quotedZshDir}
`,
    PRIVATE_FILE_MODE,
  );
  writeFileIfChanged(
    ...
    `# Luminor zsh profile wrapper
_luminor_home="\${LUMINOR_ORIGINAL_ZDOTDIR:-$HOME}"
export ZDOTDIR="$_luminor_home"
[[ -f "$_luminor_home/.zprofile" ]] && source "$_luminor_home/.zprofile"
export ZDOTDIR=${quotedZshDir}
`,
    PRIVATE_FILE_MODE,
  );
...
function applyManagedTerminalWrapperEnvState(
  ...
  return {
    ...env,
    LUMINOR_MANAGED_BIN_DIR: wrapperState.binDir,
    LUMINOR_ORIGINAL_ZDOTDIR: env.ZDOTDIR ?? env.HOME ?? "",
