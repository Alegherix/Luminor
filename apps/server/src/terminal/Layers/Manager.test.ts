...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-terminal-"));
  ...
  it("keeps a running terminal alive when open reattaches with different cwd or env", async () => {
    ...
    const snapshot = await manager.open(
      openInput({ cwd: logsDir, env: { LUMINOR_TERMINAL_TEST: "changed" } }),
    );
    ...
    if (process.platform === "win32") {
      ...
      expect(
        ptyAdapter.spawnInputs.some((input) =>
          [process.env.SHELL, "/bin/zsh", "/bin/bash", "/bin/sh", "zsh", "bash", "sh"].includes(
            input.shell,
          ),
        ),
      ).toBe(true);
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.LUMINOR_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    expect(spawnInput.env.LUMINOR_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.LUMINOR_WORKTREE_PATH).toBe("/repo/worktree-a");
