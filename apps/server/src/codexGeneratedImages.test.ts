...
import type { ProviderRuntimeEvent } from "@luminor/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousLuminorHome === undefined) delete process.env.LUMINOR_HOME;
    else process.env.LUMINOR_HOME = previousLuminorHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.LUMINOR_HOME = "/luminor-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/luminor-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.luminor/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.luminor/runtime/codex-home-overlay";
