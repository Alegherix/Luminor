...
const layer = ExternalMcpServiceLive.pipe(
  ...
  Layer.provide(Layer.succeed(ServerConfig, { baseDir: "/tmp/luminor-test" } as never)),
);
  ...
  it("issues one pairing exchange, verifies it, and revokes it immediately", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(created.stdio.args).toContain("/tmp/luminor-test");
    ...
    await run(
      Effect.gen(function* () {
        ...
        const auditId = yield* service.beginAudit(client, {
          tool: "luminor_list_allowed_projects",
        });
        yield* sql`
