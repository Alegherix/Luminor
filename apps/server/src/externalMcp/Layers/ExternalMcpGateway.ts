...
const EXTERNAL_MCP_INSTRUCTIONS =
  "This is Luminor's loopback-only external integration. Call luminor_overview first to discover the allowed projects (with on-disk paths), provider availability, and granted scopes. Tools are restricted to the integration's allowed projects and scopes. Task creation is one task per stable requestId and defaults to a managed worktree with approval-required execution.";
...
export const makeExternalMcpGateway = Effect.gen(function* () {
  ...
  const capabilitiesTool: ExternalTool = {
    ...
    definition: {
      ...
      annotations: { title: "Luminor integration capabilities", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
  ...
  const projectsTool: ExternalTool = {
    requiredCapability: "projects:read",
    definition: {
      name: "luminor_list_allowed_projects",
      description: "List only the Luminor projects explicitly granted to this integration.",
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: { title: "List allowed Luminor projects", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
  ...
  const overviewTool: ExternalTool = {
    requiredCapability: "projects:read",
    definition: {
      name: "luminor_overview",
      description:
        "Discover everything this integration can use in one call: every allowed Luminor project with its on-disk path and activity, provider availability, granted scopes, and safe defaults. Call this first to orient yourself.",
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: { title: "Luminor overview", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (_args, context) =>
      Effect.gen(function* () {
  ...
  const createTaskTool: ExternalTool = {
    ...
    definition: {
      name: "luminor_create_task",
      description:
        "Create exactly one Luminor task in an explicitly allowed project. requestId is a stable idempotency key and cannot be reused with a different plan. Defaults to a managed worktree and approval-required runtime.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  const readTaskTool: ExternalTool = {
    ...
    definition: {
      ...
      annotations: { title: "Read a permitted Luminor task", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
  ...
  const waitTaskTool: ExternalTool = {
    ...
    definition: {
      ...
      annotations: { title: "Wait for a permitted Luminor task", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        ...
        return mcpToolResultJson({
          ...
          summaryTruncated,
          error: failure,
          readTask: { tool: "luminor_read_task", arguments: { threadId: input.threadId } },
        });
      }).pipe(Effect.catch((error) => Effect.succeed(externalErrorResult(error)))),
  };
  ...
  const handleRequest = (
    ...
    return Effect.gen(function* () {
      ...
      yield* auditCompletion.complete({
        ...
        createdTaskIds: toolName === "luminor_create_task" ? createdThreadIds(result) : [],
