...
function makeBaseDir() {
  const value = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-mcp-bridge-test-"));
...
describe("external MCP stdio bridge", () => {
  ...
  it("passes hostile Windows runtime paths outside the encoded PowerShell program", () => {
    ...
    expect(invocation.options.env.LUMINOR_RUNTIME_ACL_TARGET).toBe(targetPath);
    ...
    expect(
      requestTimeoutForBody(
        JSON.stringify({
          ...
          id: 1,
          method: "tools/call",
          params: { name: "luminor_wait_for_task", arguments: { threadId: "thread-1" } },
        }),
      ),
    ).toBe(35_000);
    expect(
      ...
      requestTimeoutForBody(
        JSON.stringify([
          ...
          },
          {
            ...
            id: 4,
            method: "tools/call",
            params: { name: "luminor_wait_for_task", arguments: { threadId: "thread-2" } },
          },
        ]),
      ),
    ).toBe(65_000);
    expect(
      requestTimeoutForBody(
        JSON.stringify({
          ...
          id: 5,
          method: "tools/call",
          params: { name: "luminor_create_task", arguments: {} },
        }),
      ),
    ).toBe(605_000);
  });
  ...
  it("fails clearly for missing and multiple running instances", () => {
    ...
    expect(() => discoverExternalMcpRuntime(baseDir)).toThrow(/No running Luminor instance/);
    writeRuntime(baseDir, "userdata", 3773);
    writeRuntime(baseDir, "dev", 4773);
    expect(() => discoverExternalMcpRuntime(baseDir)).toThrow(/Multiple running Luminor instances/);
  });
    ...
    await serveExternalMcpStdio({
      ...
      fetchImpl: async (url, init) => {
        ...
        if (urls.length === 1) {
          ...
          throw new TypeError("old Luminor instance stopped");
        }
  ...
  it("allows a fast request to complete while a wait request is still pending", async () => {
    ...
    await serveExternalMcpStdio({
      baseDir,
      stdin: Readable.from([
        `${JSON.stringify({ jsonrpc: "2.0", id: "slow", method: "tools/call", params: { name: "luminor_wait_for_task", arguments: { timeoutMs: 100 } } })}\n`,
  ...
  it("aborts an active HTTP request when the MCP client cancels its request id", async () => {
    ...
    stdin.write(
      `${JSON.stringify({ jsonrpc: "2.0", id: "slow", method: "tools/call", params: { name: "luminor_wait_for_task", arguments: { threadId: "thread-1" } } })}\n`,
    );
