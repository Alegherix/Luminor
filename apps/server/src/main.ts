...
const DESKTOP_SHUTDOWN_TOKEN_ENV_KEY = "LUMINOR_DESKTOP_SHUTDOWN_TOKEN";
...
interface CliInput {
  ...
  readonly luminorHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  mode: Config.string("LUMINOR_MODE").pipe(
    Config.option,
    Config.map(
  ...
  desktopShutdownToken: Config.string("LUMINOR_DESKTOP_SHUTDOWN_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  logProviderEvents: optionalBooleanEnvironmentConfig("LUMINOR_LOG_PROVIDER_EVENTS"),
    ...
    Effect.gen(function* () {
      ...
      if (configuredPublicUrl && publicUrl === undefined) {
        return yield* new StartupError({
          message:
            "LUMINOR_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://luminor.example.com).",
        });
      }
      ...
      yield* Effect.try({
        try: () => preparePrivateServerPaths(derivedPaths),
          ...
          new StartupError({ message: "Failed to secure Luminor's local state directory", cause }),
      });
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Luminor running", makeServerStartupLogData(config));
      ...
      if (config.allowInsecureRemote && !config.publicUrl) {
        yield* Effect.logWarning(
          "INSECURE REMOTE ACCESS ENABLED: credentials and session traffic are unencrypted",
          {
            ...
            hint: "Use only on a trusted LAN. Configure LUMINOR_PUBLIC_URL behind HTTPS for protected remote access.",
          },
        );
      }
...
const luminorHomeFlag = Flag.string("home-dir").pipe(
...
const publicUrlFlag = Flag.string("public-url").pipe(
  ...
  Flag.withDescription(
    "HTTPS public root origin provided by a TLS-terminating reverse proxy (equivalent to LUMINOR_PUBLIC_URL).",
  ),
  Flag.optional,
);
...
const logProviderEventsFlag = optionalBooleanFlag("log-provider-events", {
  description:
    "Emit native/canonical provider NDJSON logs for debugging (equivalent to LUMINOR_LOG_PROVIDER_EVENTS).",
});
const logWebSocketEventsFlag = optionalBooleanFlag("log-websocket-events", {
  description:
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to LUMINOR_LOG_WS_EVENTS).",
...
// Base `luminor` command defined before the MCP subcommands so they can yield
// its parsed input (notably `--home-dir` / `luminorHome`) via Effect's command
...
const baseServerCommand = Command.make("luminor", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  luminorHome: luminorHomeFlag,
  devUrl: devUrlFlag,
...
}).pipe(Command.withDescription("Run the Luminor server."));
...
const mcpServeCommand = Command.make(
  ...
  ({ integration }) =>
    Effect.gen(function* () {
      ...
      const baseDir = resolveExternalMcpBaseDir(Option.getOrUndefined(parent.luminorHome));
  ...
  Command.withDescription(
    "Serve the paired Luminor external MCP integration over stdio for Codex, Claude, and other MCP clients.",
  ),
);
...
const mcpPairCommand = Command.make(
  "pair",
  {
    code: Flag.string("code").pipe(
      Flag.withDescription("Short-lived pairing code issued by Luminor Settings."),
    ),
  },
  ({ code }) =>
    Effect.gen(function* () {
      ...
      process.stdout.write(
        `Paired Luminor external MCP integration "${paired.paired.name}".\nCredential stored privately at ${paired.storePath}.\nConfigure the MCP client command as: ${externalMcpShellCommand(externalMcpLauncher(["mcp", "serve", "--integration", paired.paired.integrationId, "--home-dir", baseDir]))}\n`,
      );
      if (process.platform === "win32") {
        process.stdout.write(
          "Windows note: Luminor stores this credential under your user profile, but Windows does not expose POSIX 0600 permission checks. Protect the profile and its Luminor data directory.\n",
        );
      }
    }),
).pipe(Command.withDescription("Pair this CLI with a user-approved Luminor MCP integration."));

const mcpCommand = Command.make("mcp").pipe(
  Command.withDescription("Manage Luminor's loopback external MCP bridge."),
...
export const luminorCli = serverCommand;
