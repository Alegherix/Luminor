...
describe("linkOrCopyCodexOverlayEntry", () => {
  it("copies auth.json when symlink creation is unavailable", async () => {
    ...
    await linkOrCopyCodexOverlayEntry(
      {
        ...
        targetPath: "C:\\Users\\test\\.luminor\\codex-home-overlay\\auth.json",
        type: "file",
      },
      { symlink, copyFile },
    );
    ...
    expect(symlink).toHaveBeenCalledWith(
      ...
      "C:\\Users\\test\\.luminor\\codex-home-overlay\\auth.json",
      "file",
    );
      ...
      linkOrCopyCodexOverlayEntry(
        {
          ...
          targetPath: "C:\\Users\\test\\.luminor\\codex-home-overlay\\sessions",
          type: "dir",
        },
...
describe("buildCodexProcessEnv", () => {
  it("replaces a user-defined Luminor MCP table only inside the session overlay", async () => {
    const sourceHome = mkdtempSync(path.join(os.tmpdir(), "luminor-codex-source-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "luminor-codex-runtime-"));
    const sourceConfig = [
      'model = "gpt-5.5"',
      "",
      "[mcp_servers.luminor]",
      ...
      "",
      "[mcp_servers.luminor.headers]",
      'Authorization = "stale-inline-secret"',
      "",
      "[mcp_servers.luminor.env]",
      'STALE_GATEWAY_TOKEN = "stale-inline-secret"',
      "",
      "[mcp_servers.luminor-other]",
      'url = "http://127.0.0.1:2111/luminor-other"',
      "",
    ...
    const managedConfig = [
      ...
      'bearer_token_env_var = "LUMINOR_AGENT_GATEWAY_TOKEN"',
      "",
      "[shell_environment_policy]",
      'exclude = ["LUMINOR_AGENT_GATEWAY_TOKEN"]',
    ].join("\n");
    ...
    try {
      ...
      if (!overlayHome) {
        throw new Error("Expected a Luminor Codex home overlay.");
      }
      const overlayConfig = readFileSync(path.join(overlayHome, "config.toml"), "utf8");

      expect(overlayConfig.match(/^\[mcp_servers\.luminor\]$/gm)).toHaveLength(1);
      expect(overlayConfig).toContain('url = "http://127.0.0.1:3773/mcp"');
      expect(overlayConfig).toContain('bearer_token_env_var = "LUMINOR_AGENT_GATEWAY_TOKEN"');
      ...
      expect(overlayConfig).not.toContain("[mcp_servers.luminor.headers]");
      expect(overlayConfig).not.toContain("[mcp_servers.luminor.env]");
      ...
      expect(overlayConfig).toContain('exclude = ["LUMINOR_AGENT_GATEWAY_TOKEN", "USER_SECRET"]');
