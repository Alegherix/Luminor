...
function classifyCloneFailure(cause: unknown): GitHubProjectProvisioningError {
  ...
  if (exceededConfiguredCloneTimeout) {
    return provisioningError(
      "CLONE_TIMEOUT",
      "The repository clone exceeded Luminor's 30-minute limit. For very large repositories, clone it manually and add the local folder instead.",
      false,
      cause,
    );
  }
  ...
  if (
    ...
    return provisioningError(
      "NETWORK_ERROR",
      "Luminor could not reach GitHub. Check the server's network connection and retry.",
      true,
      cause,
    );
  }
  ...
  if (lower.includes("permission denied") || lower.includes("operation not permitted")) {
    return provisioningError(
      "PERMISSION_DENIED",
      "Luminor does not have permission to write to the selected destination.",
      false,
      cause,
    );
  }
...
function classifyPromotionFailure(cause: unknown): GitHubProjectProvisioningError {
  ...
  if (reason === "PermissionDenied") {
    return provisioningError(
      "PERMISSION_DENIED",
      "Luminor does not have permission to move the cloned repository into the selected destination.",
      false,
      cause,
    );
  }
  ...
  const provisionCheckout: GitHubProjectProvisioner["provisionCheckout"] = (input, reporter) =>
    Effect.gen(function* () {
      ...
      if (!path.isAbsolute(expandedParent)) {
        return yield* provisioningError(
          "INVALID_DESTINATION",
          "Choose an absolute destination folder on the Luminor server.",
          false,
        );
      }
      ...
      return yield* withDestinationLock(
        ...
        Effect.gen(function* () {
          ...
          const stagingPath = path.join(
            parent,
            `.luminor-clone-${process.pid}-${randomUUID().replace(/-/g, "")}`,
          );
          let promoted = false;
