...
import { CommandId, ProjectId, type GitHubProjectProvisionInput } from "@luminor/contracts";
  ...
  it("uses authenticated GitHub CLI cloning without forcing SSH or HTTPS", async () => {
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const parent = yield* fileSystem.makeTempDirectoryScoped({ prefix: "luminor-provision-" });
    ...
    expect(result.ghCalls).toEqual([
      [
        ...
        "--no-upstream",
        "openai/codex",
        expect.stringContaining(".luminor-clone-"),
        "--",
        "--progress",
      ],
    ]);
