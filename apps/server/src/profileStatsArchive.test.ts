...
import { MessageId, ThreadId, TurnId } from "@luminor/contracts";
...
const testLayer = Layer.mergeAll(ProfileStatsQueryLive, ProfileStatsArchiveLive).pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "luminor-profile-stats-archive-test-",
    }),
  ),
      ...
      Effect.gen(function* () {
        ...
        yield* sql`
          INSERT INTO external_mcp_integrations (
            ...
            'integration-purge', 'Purge integration', 'luminor.external-mcp', 'other',
