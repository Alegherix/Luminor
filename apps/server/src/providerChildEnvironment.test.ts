...
describe("buildProviderChildEnvironment", () => {
  it("strips Luminor control-plane and inherited native capabilities", () => {
    const env = buildProviderChildEnvironment({
      ...
      baseEnv: {
        ...
        LUMINOR_AUTH_TOKEN: "control-plane-secret",
        LUMINOR_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
        LUMINOR_BROWSER_HOST_CAPABILITY: "private-desktop-capability",
  ...
  it("admits only explicitly granted capability keys", () => {
    const env = buildProviderChildEnvironment({
      ...
      inheritedLuminorKeys: ["LUMINOR_BROWSER_USE_PIPE_PATH"],
  ...
  it("does not let overlays bypass the capability policy", () => {
    const env = buildProviderChildEnvironment({
      ...
      overrides: {
        ...
        LUMINOR_AUTH_TOKEN: "overlaid-control-plane-secret",
        NODE_OPTIONS: "--require=/tmp/inject.js",
      },
    });
  ...
  it("keeps stripped authority absent in descendants", () => {
    ...
    const descendantScript =
      "process.stdout.write(JSON.stringify({ xai: process.env.XAI_API_KEY, anthropic: process.env.ANTHROPIC_API_KEY, luminor: process.env.LUMINOR_AUTH_TOKEN }))";
