...
describe("auth utils", () => {
  it("resolves stable web and port-scoped desktop cookie names", () => {
    expect(resolveSessionCookieName({ mode: "web", port: 3773 })).toBe("luminor_session");
    expect(resolveSessionCookieName({ mode: "desktop", port: 3773 })).toBe("luminor_session_3773");
  });
