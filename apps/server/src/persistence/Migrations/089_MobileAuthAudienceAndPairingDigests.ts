...
export default Effect.gen(function* () {
  ...
  if (yield* tableExists(sql, "auth_pairing_links")) {
    ...
    if (!alreadyRebuilt) {
      yield* sql`DROP TABLE IF EXISTS auth_pairing_links_v89`;
      yield* sql`
        CREATE TABLE auth_pairing_links_v89 (
          id TEXT PRIMARY KEY,
      ...
      yield* sql`
        INSERT INTO auth_pairing_links_v89 (
          id,
          credential_digest,
          credential_hint,
          audience,
      ...
      yield* sql`ALTER TABLE auth_pairing_links_v89 RENAME TO auth_pairing_links`;
    }
