...
layer("032_ReconcileImportedSchemaLineage", (it) => {
  // Simulates a legacy ~/.luminor import where the imported `effect_sql_migrations`
  // tracker has IDs 17-31 recorded under unrelated Luminor names. The 17-31
  ...
  // references env_mode.
  it.effect("heals an imported Luminor DB whose tracker skipped 17-31", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;

      // Bring the schema to where Luminor and Luminor last agreed.
      yield* runMigrations({ toMigrationInclusive: 16 });

      // Mark IDs 17-31 applied under Luminor's old names so the migrator
      // skips Luminor's renumbered 17-23. Names are illustrative; only the
      ...
      // Seed a thread row with the Luminor-era column set so the data-rewrite
  ...
  it.effect("is a no-op on a fresh Luminor install", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
