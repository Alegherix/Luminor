...
} from "@luminor/shared/migrationRecovery";
export {
  migrationBackupDirectory,
  migrationRecoveryMarkerPath,
} from "@luminor/shared/migrationRecovery";
...
export class MigrationRecoveryRequiredError extends Error {
  ...
  constructor(
    ...
    super(
      `Migration recovery is required for ${dbPath}.${detail ? ` ${detail}` : ""} Stop every Luminor process, then run: luminor-restore-migration-backup ${shellQuote(dbPath)}`,
    );
  ...
  constructor(
    ...
    super(
      `Not enough free disk space to back up the database before upgrading it. ` +
        ...
        `${formatBytes(availableBytes)} is free. Free up disk space and start Luminor again.`,
    );
...
const writeRecoveryMarker = (dbPath: string, backup: MigrationBackupResult) =>
  attemptPromise(async () => {
    await writeRecoveryMarkerFile(migrationRecoveryMarkerPath(dbPath), {
      ...
      resumeAttempts: 0,
      recovery:
        "Luminor retries an interrupted migration a bounded number of times on startup. Once that budget is spent it refuses to open this database until an operator stops every Luminor process and runs the explicit migration-backup restore command.",
    });
  });
...
/**
 * Explicit one-shot recovery path. The operator must stop every Luminor process
