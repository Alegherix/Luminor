...
import Migration0090 from "./Migrations/090_Folders.ts";
import Migration0091 from "./Migrations/091_ThreadFolderMembership.ts";
import Migration0092 from "./Migrations/092_SpaceFolderOwnership.ts";
...
export const migrationEntries = [
  ...
  [90, "Folders", Migration0090],
  [91, "ThreadFolderMembership", Migration0091],
  [92, "SpaceFolderOwnership", Migration0092],
] as const;
...
/**
 * A tracker identity that a *released* Luminor build wrote for a migration whose
 ...
 * does the migration Luminor registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Luminor's latest ID,
 * every Luminor migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Luminor's lineage and
 ...
 *
 * Divergences caused by Luminor renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Luminor build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Luminor migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Luminor build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Luminor lineage (legacy import); re-running migrations from the divergence point",
