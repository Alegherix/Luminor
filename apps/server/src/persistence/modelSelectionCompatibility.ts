...
import { MODEL_OPTIONS_BY_PROVIDER } from "@luminor/contracts";
...
// Imported instance ids may be runtime names rather than Luminor provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Luminor writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Luminor stores canonical provider/options objects.
