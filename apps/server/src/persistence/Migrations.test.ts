...
layer("reconcileMigrationLineage", (it) => {
  ...
  // mark is at or beyond Luminor's latest migration ID. The migrator's max-ID
  // gate then skips every Luminor migration — including the #032 self-heal —
  // and startup crashes on the missing env_mode column.
  it.effect("re-runs skipped migrations when an imported tracker outruns Luminor's latest ID", () =>
    Effect.gen(function* () {
      ...
      // Record a foreign lineage from 17 through past Luminor's latest ID.
      const latestLuminorId = Math.max(...migrationEntries.map(([id]) => id));
      for (let id = 17; id <= latestLuminorId + 3; id++) {
        yield* sql`
      ...
      // The tracker now mirrors the Luminor lineage exactly; foreign rows are gone.
  ...
  it.effect("refuses writable migration startup for a newer Luminor schema", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES (${futureId}, 'FutureLuminorMigration')
      `;
  ...
  it.effect("keeps a private database that already recorded old migration 54 compatible", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(executed, [
        ...
        [90, "Folders"],
        [91, "ThreadFolderMembership"],
        [92, "ProjectScriptKinds"],
        [93, "SpaceFolderOwnership"],
        [94, "AgentGatewaySequentialCreationPlans"],
        [95, "PullRequestInboxState"],
        [96, "ProjectionThreadMessageTextSegments"],
        [97, "AutomationFailureTolerance"],
        [98, "BackfillAutomationRunThreadSource"],
        [99, "BackfillMaxIterationsDisabledReason"],
        [100, "ProjectionThreadsGoal"],
        [101, "ProjectionThreadsGoalTiming"],
        [102, "ProjectionThreadsGoalAchievements"],
      ]);
      ...
      assert.deepStrictEqual(tracker.slice(-48), [
        ...
        { migration_id: 90, name: "Folders" },
        { migration_id: 91, name: "ThreadFolderMembership" },
        { migration_id: 92, name: "ProjectScriptKinds" },
        { migration_id: 93, name: "SpaceFolderOwnership" },
        { migration_id: 94, name: "AgentGatewaySequentialCreationPlans" },
        { migration_id: 95, name: "PullRequestInboxState" },
        { migration_id: 96, name: "ProjectionThreadMessageTextSegments" },
        { migration_id: 97, name: "AutomationFailureTolerance" },
        { migration_id: 98, name: "BackfillAutomationRunThreadSource" },
        { migration_id: 99, name: "BackfillMaxIterationsDisabledReason" },
        { migration_id: 100, name: "ProjectionThreadsGoal" },
        { migration_id: 101, name: "ProjectionThreadsGoalTiming" },
        { migration_id: 102, name: "ProjectionThreadsGoalAchievements" },
      ]);
