...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("luminor-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("luminor-codex-home-");
  ...
  it("allows images written to the LUMINOR_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Luminor overlay
    // directory (see resolveLuminorCodexHomeOverlayPath). Generated images therefore
    // live under <LUMINOR_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const luminorHome = path.join(fakeRoot, "luminor", "runtime");
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("luminor-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/luminor-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "luminor-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("luminor-pdf-outside-");
