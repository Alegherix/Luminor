...
describe("resolveOutOfRootFileReference", () => {
  ...
  beforeEach(() => {
    homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-out-of-root-"));
  ...
  it("never walks above the home directory", async () => {
    ...
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-outside-home-"));
    try {
  ...
  it("returns null when the workspace root lives outside the home directory", async () => {
    const foreignRoot = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-foreign-root-"));
    try {
      const resolved = await resolveOutOfRootFileReference({
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-symlink-escape-"));
      try {
        writeFile(path.join(outsideDir, "escape.md"));
