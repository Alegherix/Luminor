...
import { CliConfig, makeServerStartupLogData, luminorCli, type CliConfigShape } from "./main";
...
let defaultLuminorHome = "";
const tempHomes = new Set<string>();

function makeTempHome(prefix = "luminor-main-test-"): string {
...
const testLayer = Layer.mergeAll(
  Layer.succeed(CliConfig, {
    cwd: "/tmp/luminor-test-workspace",
    fixPath: Effect.void,
...
const runCli = (args: ReadonlyArray<string>, env: Record<string, string> = {}) => {
  const program = Command.runWith(luminorCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            LUMINOR_HOME: defaultLuminorHome,
...
beforeEach(() => {
  vi.clearAllMocks();
  defaultLuminorHome = makeTempHome();
  resolvedConfig = null;
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("luminor-main-flag-");
  ...
  it.effect("passes the root --home-dir flag to MCP subcommands", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("luminor-main-mcp-flag-");
  ...
  it.effect("creates fresh local state directories with private permissions", () =>
    Effect.gen(function* () {
      ...
      const homeDir = makeTempHome("luminor-main-private-fresh-");
  ...
  it.effect("repairs permissions for an upgraded local state directory", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return;
      const homeDir = makeTempHome("luminor-main-private-upgrade-");
      const stateDir = path.join(homeDir, "userdata");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      ...
      yield* runCli([], {
        ...
        LUMINOR_HOME: envHome,
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        LUMINOR_NO_BROWSER: "true",
        LUMINOR_AUTH_TOKEN: "env-token",
        LUMINOR_DESKTOP_SHUTDOWN_TOKEN: "shutdown-token",
      });
  ...
  it.effect("consumes desktop shutdown authority before generic child launches", () =>
    Effect.gen(function* () {
      const canonicalKey = "LUMINOR_DESKTOP_SHUTDOWN_TOKEN";
      const mixedCaseKey = "Luminor_dEsKtOp_ShUtDoWn_ToKeN";
      ...
      try {
        ...
        resolvedConfig = null;
        yield* runCli([], { LUMINOR_DESKTOP_SHUTDOWN_TOKEN: injectedToken });
  ...
  it.effect("omits both server authority secrets from startup log data", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        LUMINOR_AUTH_TOKEN: "browser-secret",
        LUMINOR_DESKTOP_SHUTDOWN_TOKEN: "shutdown-secret",
      });
  ...
  it.effect("prefers --mode over LUMINOR_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over LUMINOR_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        LUMINOR_NO_BROWSER: "false",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        ...
        ],
        {
          LUMINOR_MODE: "desktop",
          LUMINOR_NO_BROWSER: "true",
          LUMINOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
          LUMINOR_LOG_PROVIDER_EVENTS: "true",
          LUMINOR_LOG_WS_EVENTS: "true",
        },
      );
  ...
  it.effect("honors insecure remote opt-in from the environment when the CLI flag is absent", () =>
    Effect.gen(function* () {
      yield* runCli(["--host", "0.0.0.0", "--auth-token", "remote-secret"], {
        LUMINOR_ALLOW_INSECURE_REMOTE: "true",
        LUMINOR_NO_BROWSER: "true",
      });
  ...
  it.effect("uses the HTTPS public origin for remote startup pairing", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.publicUrl?.origin, "https://luminor.example.test");
      assert.equal(openBrowser.mock.calls.length, 1);
      assert.match(
        openBrowser.mock.calls[0]?.[0] ?? "",
        /^https:\/\/luminor\.example\.test\/pair#token=/,
      );
    }),
  );
  ...
  it.effect("supports the HTTPS public origin through environment configuration", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        LUMINOR_HOST: "192.168.1.50",
        LUMINOR_AUTH_TOKEN: "remote-secret",
        LUMINOR_PUBLIC_URL: "https://luminor.example.test",
      });
  ...
  it.effect("rejects non-root or non-HTTPS public URLs", () =>
    Effect.gen(function* () {
      for (const publicUrl of ["http://luminor.example.test", "https://luminor.example.test/app"]) {
        const error = yield* Effect.flip(
  ...
  it.effect("refuses non-loopback exposure without authentication", () =>
    Effect.gen(function* () {
      ...
      assert.match(String(error), /Refusing to bind Luminor to non-loopback host 0\.0\.0\.0/);
    }),
  );
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        LUMINOR_MODE: "desktop",
        LUMINOR_LOG_PROVIDER_EVENTS: "true",
        LUMINOR_LOG_WS_EVENTS: "false",
        LUMINOR_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        LUMINOR_NO_BROWSER: "true",
      });
  ...
  it.effect("rejects invalid boolean environment values instead of treating them as absent", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
        runCli([], {
          LUMINOR_LOG_PROVIDER_EVENTS: "sometimes",
        }),
      );
