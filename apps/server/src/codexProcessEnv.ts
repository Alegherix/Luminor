...
// Purpose: Builds the exact environment used when Luminor launches Codex subprocesses.
...
import { readActiveCodexProviderEnvKey } from "@luminor/shared/codexConfig";
import {
  readEnvironmentFromLoginShell,
  resolveLoginShell,
  type ShellEnvironmentReader,
} from "@luminor/shared/shell";

import { resolveBaseCodexHomePath, resolveLuminorCodexHomeOverlayPath } from "./codexHomePaths.ts";
import { buildProviderChildEnvironment } from "./providerChildEnvironment.ts";

const CODEX_PROCESS_SHELL_ENV_NAMES = ["PATH", "SSH_AUTH_SOCK"] as const;
const CODEX_OVERLAY_SHARED_STATE_FILES = new Set(["auth.json"]);
const LUMINOR_CONFIG_SUPPRESSIONS_FILE = "luminor-config-suppressions-v1.json";
const LUMINOR_MANAGED_MCP_TABLE_HEADER = "[mcp_servers.luminor]";
export const LUMINOR_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS = [
...
async function writeLuminorConfigSuppressions(
  markerPath: string,
  sectionHeaders: readonly string[],
): Promise<void> {
...
export const LUMINOR_MANAGED_CODEX_CONFIG_BEGIN = "# >>> luminor managed config >>>";
export const LUMINOR_MANAGED_CODEX_CONFIG_END = "# <<< luminor managed config <<<";

export function extractManagedCodexConfigSection(config: string): string | undefined {
  const begin = config.indexOf(LUMINOR_MANAGED_CODEX_CONFIG_BEGIN);
  if (begin === -1) {
    return undefined;
  }
  const contentStart = begin + LUMINOR_MANAGED_CODEX_CONFIG_BEGIN.length;
  const end = config.indexOf(LUMINOR_MANAGED_CODEX_CONFIG_END, contentStart);
  if (end === -1) {
    return undefined;
  }
...
function appendManagedCodexConfigSection(config: string, section: string): string {
  ...
  const managedMcpTableName = normalizeTomlTableHeaderName(LUMINOR_MANAGED_MCP_TABLE_HEADER);
  const tables: string[] = [];
  ...
  for (const table of splitTomlTables(section.trim())) {
    ...
    if (normalizeTomlTableHeaderName(header) === managedMcpTableName) {
      // The session-scoped gateway entry is authoritative inside Luminor's
      // overlay. The user's source config remains untouched.
      overlayConfig = removeTomlTableNamespace(overlayConfig, LUMINOR_MANAGED_MCP_TABLE_HEADER);
      tables.push(table);
      continue;
    }
...
async function prepareLuminorCodexHomeOverlayUnlocked(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  const sourceHomePath = resolveBaseCodexHomePath(input.env, input.homePath);
  const overlayHomePath = resolveLuminorCodexHomeOverlayPath(input.env, sourceHomePath);
  ...
  const suppressionMarkerPath = path.join(overlayHomePath, LUMINOR_CONFIG_SUPPRESSIONS_FILE);
  const suppressedSections = [
    ...new Set([
      ...
      ...(await readLuminorConfigSuppressions(suppressionMarkerPath)),
    ]),
  ...
  await writeLuminorConfigSuppressions(suppressionMarkerPath, suppressedSections);

  return overlayHomePath;
}

async function prepareLuminorCodexHomeOverlay(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  ...
  return serializeCodexOverlayPreparation(overlayHomePath, () =>
    prepareLuminorCodexHomeOverlayUnlocked(input),
  );
}
...
export async function buildCodexProcessEnv(
  ...
  const overlayHomePath = await prepareLuminorCodexHomeOverlay({
    env: baseEnv,
