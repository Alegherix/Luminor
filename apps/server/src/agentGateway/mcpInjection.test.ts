...
import {
  ...
  LUMINOR_MANAGED_CODEX_CONFIG_BEGIN,
  LUMINOR_MANAGED_CODEX_CONFIG_END,
} from "../codexProcessEnv.ts";
import {
  buildAntigravityMcpPluginConfig,
  buildAcpLuminorMcpServers,
...
describe("agent gateway MCP injection", () => {
  ...
  it("builds a codex config block that references the token env var, not the token", () => {
    ...
    assert.include(block, "[mcp_servers.luminor]");
    assert.include(block, `url = "${connection.url}"`);
    assert.include(block, `bearer_token_env_var = "${LUMINOR_AGENT_GATEWAY_TOKEN_ENV}"`);
  ...
  it("appends the codex section once and keeps existing config intact", () => {
    ...
    assert.include(appended, "[mcp_servers.luminor]");

    const reappended = appendCodexConfigSection(appended, section);
    assert.equal(reappended.split("[mcp_servers.luminor]").length, 2);
  });
  ...
  it("merges the token exclusion into a user-defined shell environment policy", () => {
    ...
    const merged = mergeShellEnvPolicyExclude(withExclude, LUMINOR_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${LUMINOR_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`);

    // Idempotent: the var is not added twice.
    assert.equal(mergeShellEnvPolicyExclude(merged, LUMINOR_AGENT_GATEWAY_TOKEN_ENV), merged);

    // A policy table without an exclude key gains one.
    const withoutExclude = ["[shell_environment_policy]", 'inherit = "core"'].join("\n");
    const gained = mergeShellEnvPolicyExclude(withoutExclude, LUMINOR_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(gained, `exclude = ["${LUMINOR_AGENT_GATEWAY_TOKEN_ENV}"]`);
    assert.include(gained, 'inherit = "core"');

    // No policy table: unchanged (the managed section appends its own).
    assert.equal(
      mergeShellEnvPolicyExclude('[model]\nname = "gpt-5.5"', LUMINOR_AGENT_GATEWAY_TOKEN_ENV),
      '[model]\nname = "gpt-5.5"',
    );
  });
  ...
  it("ignores commented and unrelated token references when merging shell exclusions", () => {
    const commentedExample = [
      "[shell_environment_policy]",
      `# exclude = ["${LUMINOR_AGENT_GATEWAY_TOKEN_ENV}"]`,
      'exclude = ["AWS_*"]',
    ].join("\n");
    ...
    assert.include(
      ...
      `exclude = ["${LUMINOR_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`,
    );

    const unrelatedString = [
      "[shell_environment_policy]",
      `note = "keep ${LUMINOR_AGENT_GATEWAY_TOKEN_ENV} private"`,
      'exclude = ["AWS_*"]',
    ].join("\n");
  ...
  it("recognizes only active exact entries in multiline shell exclusion arrays", () => {
    ...
    assert.equal(mergeShellEnvPolicyExclude(existing, LUMINOR_AGENT_GATEWAY_TOKEN_ENV), existing);
    ...
    const merged = mergeShellEnvPolicyExclude(tokenOnlyInComment, LUMINOR_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${LUMINOR_AGENT_GATEWAY_TOKEN_ENV}",`);
  });

  it("detects real TOML table headers, ignoring comments and strings", () => {
    assert.isTrue(
      configHasTomlTableHeader('[mcp_servers.luminor]\nurl = "x"', "[mcp_servers.luminor]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [mcp_servers.luminor]  # managed", "[mcp_servers.luminor]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [ mcp_servers.luminor ]  # managed", "[mcp_servers.luminor]"),
    );
    ...
    assert.isFalse(configHasTomlTableHeader('["mcp_servers.luminor"]', "[mcp_servers.luminor]"));
    assert.isFalse(configHasTomlTableHeader('[mcp_servers."syn\\qara"]', "[mcp_servers.luminor]"));
    // A commented-out example block must not count as the table being present.
    assert.isFalse(configHasTomlTableHeader("# [mcp_servers.luminor]", "[mcp_servers.luminor]"));
    assert.isFalse(
      configHasTomlTableHeader('note = "see [mcp_servers.luminor] docs"', "[mcp_servers.luminor]"),
    );
  });
                ...
                tools: [
                  {
                    ...
                    description: "List Luminor threads.",
  ...
  it("uses the ACP http transport when the agent advertises support", () => {
    const servers = buildAcpLuminorMcpServers({
      connection,
    ...
    assert.deepEqual(servers, [
      {
        ...
        args: stdioProxy.args,
        env: [
          { name: "LUMINOR_AGENT_GATEWAY_URL", value: connection.url },
          { name: LUMINOR_AGENT_GATEWAY_TOKEN_ENV, value: connection.bearerToken },
        ],
      },
    ]);
  });
