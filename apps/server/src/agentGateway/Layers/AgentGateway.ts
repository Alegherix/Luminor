/**
 * AgentGatewayLive - Luminor app-control MCP tool surface.
 *
 * Implements the `luminor_*` tools served over `POST /mcp` (streamable HTTP,
 ...
 * a Luminor thread can list/read/create/steer threads and manage heartbeat
...
const AGENT_GATEWAY_INSTRUCTIONS =
  "Luminor tools are thread-scoped. Use browser_* only for Luminor's shared in-app browser runtime; follow the provider-delivered <luminor_host_context> for full policy.";
...
export const makeAgentGateway = Effect.gen(function* () {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          threads: {
            type: "array",
            minItems: 1,
            maxItems: LUMINOR_GATEWAY_MAX_THREADS_PER_OPERATION,
            items: {
              type: "object",
              properties: {
                prompt: { type: "string" },
  ...
  const createThread: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Create exactly one standalone Luminor thread. Worktree threads start on a Luminor-managed temporary branch pinned at baseRef; on the first turn Luminor may rename the branch after the prompt and publish it. For two or more threads use one luminor_create_threads call instead.",
      inputSchema: {
        type: "object",
        properties: {
  ...
  const sendMessage: ToolEntry = {
    ...
    definition: {
      ...
      description:
        'Send a Luminor follow-up message to an existing thread. mode "queue" (default) waits for the current turn; "steer" redirects a running turn where the provider supports it (otherwise it is queued).',
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Send a Luminor message", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
  ...
  const interruptThread: ToolEntry = {
    ...
    definition: {
      ...
      description: "Interrupt the running turn of a Luminor thread.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Interrupt a Luminor thread", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const threadId = readStringArg(args, "threadId", { required: true })!;
  ...
  const setThreadTitle: ToolEntry = {
    ...
    definition: {
      ...
      description: "Rename a Luminor thread.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Rename a Luminor thread", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const threadId = readStringArg(args, "threadId", { required: true })!;
  ...
  const setThreadArchived: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Archive or unarchive a Luminor thread. Defaults to your own thread when threadId is omitted.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Update a Luminor thread", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
