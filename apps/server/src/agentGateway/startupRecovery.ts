...
import { CommandId, ThreadId } from "@luminor/contracts";
...
export function recoverInterruptedAgentGatewayOperations(input: {
  ...
  readonly creationSource?: "luminor_mcp" | "external_mcp";
        ...
        Effect.gen(function* () {
          if (operation.status === "reserved") {
            yield* input.operationRepository.fail({
              ...
              errorJson: JSON.stringify({
                ...
                message:
                  "Luminor restarted before dispatch began. No git or orchestration resources were touched.",
              }),
              now: gatewayIsoNow(),
            });
            return;
            ...
            (entry) =>
              Effect.gen(function* () {
                ...
                if (Option.isSome(projected)) {
                  if (
                    projected.value.creationSource !== (input.creationSource ?? "luminor_mcp") ||
          ...
          if (recoveryErrors.length > 0) {
            yield* input.operationRepository.recordCompensationFailure({
              ...
              errorJson: JSON.stringify({
                ...
                message:
                  "Luminor could not fully compensate the interrupted operation during startup recovery. The sanitized operation remains retryable and some resources may require manual cleanup; no replacements will be created.",
          ...
          yield* input.operationRepository.fail({
            ...
            errorJson: JSON.stringify({
              ...
              message:
                "Luminor restarted before the operation completed. Deterministic operation-owned resources were compensated; no replacements were created.",
              ...
              yield* input.operationRepository
                .recordCompensationFailure({
                  ...
                  errorJson: JSON.stringify({
                    ...
                    message:
                      "Luminor could not recover the interrupted operation. The sanitized operation remains retryable and resources may require manual cleanup; no replacements will be created.",
                    error: detail,
                  }),
                  now: gatewayIsoNow(),
                })
