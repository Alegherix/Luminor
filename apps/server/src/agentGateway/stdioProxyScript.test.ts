...
describe("agent gateway stdio proxy", () => {
  it("forwards cancellation immediately and lets a later ping bypass a hung request", async () => {
    const stateDir = mkdtempSync(join(tmpdir(), "luminor-stdio-proxy-"));
    ...
    try {
      ...
      child = spawn(process.execPath, [scriptPath], {
        env: {
          ...process.env,
          LUMINOR_AGENT_GATEWAY_URL: `http://127.0.0.1:${address.port}/mcp`,
          LUMINOR_AGENT_GATEWAY_TOKEN: "test-token",
        },
  ...
  it("exchanges an ambient one-shot bootstrap and keeps the bearer inside the proxy", async () => {
    const stateDir = mkdtempSync(join(tmpdir(), "luminor-stdio-bootstrap-"));
    ...
    try {
      ...
      const providerEnvironment = {
        ...
        LUMINOR_AGENT_GATEWAY_URL: `http://127.0.0.1:${address.port}/mcp`,
        LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "one-shot-bootstrap",
      };
      child = spawn(process.execPath, [scriptPath], {
      ...
      const runProviderDescendant = async (): Promise<unknown> => {
        const descendant = spawn(
          ...
          [
            "-e",
            `const url = process.env.LUMINOR_AGENT_GATEWAY_URL + "/bootstrap";
             fetch(url, { method: "POST", headers: { Authorization: "Bearer " + process.env.LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN } })
               .then((response) => process.stdout.write(JSON.stringify({ status: response.status, bearer: process.env.LUMINOR_AGENT_GATEWAY_TOKEN ?? null })))
  ...
  it("aborts a hung eager bootstrap when stdin closes", async () => {
    const stateDir = mkdtempSync(join(tmpdir(), "luminor-stdio-bootstrap-abort-"));
    const bootstrapStarted = deferred<void>();
    let server: Server | undefined;
    let child: ChildProcessWithoutNullStreams | undefined;
    ...
    try {
      ...
      child = spawn(process.execPath, [scriptPath], {
        env: {
          PATH: process.env.PATH,
          LUMINOR_AGENT_GATEWAY_URL: `http://127.0.0.1:${address.port}/mcp`,
          LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "hung-bootstrap",
        },
        stdio: ["pipe", "pipe", "pipe"],
      });
