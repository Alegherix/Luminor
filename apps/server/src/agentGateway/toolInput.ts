...
export const MODEL_SELECTION_INPUT_SCHEMA = {
  ...
  properties: {
    ...
    model: {
      type: "string",
      description: "Exact model slug from luminor_capabilities providers[].models[].slug.",
    },
    options: {
      type: "object",
      description: AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION,
...
export function decodeCreateThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(LuminorCreateThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Luminor creation plan: ${errorText(error)}`);
  }
}

export function decodeWaitForThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(LuminorWaitForThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Luminor wait request: ${errorText(error)}`);
  }
}
