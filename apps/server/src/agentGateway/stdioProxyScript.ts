/**
 * Stdio-to-HTTP proxy script for the Luminor agent gateway.
 *
...
const STDIO_PROXY_SCRIPT = `// Luminor agent gateway stdio<->HTTP MCP proxy (generated file, do not edit).
const url = process.env.LUMINOR_AGENT_GATEWAY_URL;
let token = process.env.LUMINOR_AGENT_GATEWAY_TOKEN;
let bootstrapToken = process.env.LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
...
function localInactiveResponse(message) {
  ...
  // CLI launched outside Luminor therefore sees a valid empty MCP server
  // instead of a noisy failed integration. Luminor-managed processes receive
    ...
    return [
      {
        ...
        result: {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "luminor", version: "1.0.0" },
        },
      },
    ];
  }
  ...
  return [
    {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "Luminor is not active for this Antigravity session." },
    },
  ];
}
...
async function resolveToken() {
  ...
  if (!tokenResolution) {
    ...
    tokenResolution = (async () => {
      ...
      if (!response.ok) {
        throw new Error("Luminor gateway bootstrap failed with HTTP " + response.status);
      }
      const payload = await response.json();
      if (!isRecord(payload) || typeof payload.bearerToken !== "string") {
        throw new Error("Luminor gateway bootstrap returned an invalid response");
      }
      token = payload.bearerToken;
      bootstrapToken = undefined;
      delete process.env.LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
      return token;
    })().finally(() => {
    ...
    return [
      {
        jsonrpc: "2.0",
        id,
        error: { code: -32603, message: "Luminor gateway request failed: " + String(error) },
      },
    ];
  }
}
