/**
 * Provider-facing config builders for the Luminor agent gateway.
 *
 ...
 *
 * - Codex: `[mcp_servers.luminor]` TOML block (streamable HTTP +
...
export const LUMINOR_MCP_SERVER_NAME = "luminor";
export const LUMINOR_AGENT_GATEWAY_TOKEN_ENV = "LUMINOR_AGENT_GATEWAY_TOKEN";
export const LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV = "LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN";
export const LUMINOR_AGENT_GATEWAY_URL_ENV = "LUMINOR_AGENT_GATEWAY_URL";
 ...
 * the block references an env var that Luminor sets per app-server process.
 *
...
export function buildCodexMcpConfigToml(endpointUrl: string): string {
  return [
    `[mcp_servers.${LUMINOR_MCP_SERVER_NAME}]`,
    `url = ${JSON.stringify(endpointUrl)}`,
    `bearer_token_env_var = ${JSON.stringify(LUMINOR_AGENT_GATEWAY_TOKEN_ENV)}`,
    "",
    "[shell_environment_policy]",
    `exclude = [${JSON.stringify(LUMINOR_AGENT_GATEWAY_TOKEN_ENV)}]`,
  ].join("\n");
}
 ...
 * provider process dedicated to the owning Luminor thread or an exclusive
...
async function postAgentGatewayJsonRpc(input: {
  ...
  if (!response.ok) {
    throw new Error(`Luminor MCP request failed with HTTP ${String(response.status)}.`);
  }
  const payload: unknown = await response.json();
  if (!isRecord(payload) || payload.jsonrpc !== "2.0") {
    throw new Error("Luminor MCP returned an invalid JSON-RPC response.");
  }
  if ("error" in payload) {
    const failure = isRecord(payload.error) ? payload.error : null;
    throw new Error(failure?.message ? String(failure.message) : "Luminor MCP request failed.");
  }
  if (payload.id !== id || !("result" in payload)) {
    throw new Error("Luminor MCP returned a mismatched JSON-RPC response.");
  }
...
export async function listAgentGatewayMcpTools(input: {
  ...
  if (!isRecord(result) || !Array.isArray(result.tools)) {
    throw new Error("Luminor MCP tools/list returned an invalid tool catalog.");
  }
  return result.tools.map((value) => {
    if (
      ...
      throw new Error("Luminor MCP tools/list returned an invalid tool descriptor.");
    }
    return {
      name: value.name,
...
/**
 * Build the secret-free MCP fragment installed with Luminor's Antigravity
  ...
  return {
    mcpServers: {
      [LUMINOR_MCP_SERVER_NAME]: {
        ...
        env: {
          [LUMINOR_AGENT_GATEWAY_URL_ENV]: `$${LUMINOR_AGENT_GATEWAY_URL_ENV}`,
          [LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: `$${LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV}`,
 ...
 */
export function buildAcpLuminorMcpServers(input: {
  readonly connection: AgentGatewayMcpConnection;
  ...
  return [
    {
      ...
      env: [
        { name: LUMINOR_AGENT_GATEWAY_URL_ENV, value: input.connection.url },
        { name: LUMINOR_AGENT_GATEWAY_TOKEN_ENV, value: input.connection.bearerToken },
      ],
    },
  ];
}
