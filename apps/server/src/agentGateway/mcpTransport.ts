import { ThreadId, type OrchestrationThreadShell } from "@luminor/contracts";
        ...
        Effect.gen(function* () {
          if (callerWriteAuthority === null) {
            return yield* Effect.fail(
              new GatewayToolError(
                ...
                "This Luminor write was rejected because this credential had no write authority for the exact active turn when the MCP request arrived.",
                {
                  callerThreadId,
          ...
          if (!input.credentials.verifyWriteAuthority(callerWriteAuthority)) {
            return yield* Effect.fail(
              new GatewayToolError(
                ...
                "This Luminor write was rejected because its provider-session authority is no longer active.",
                { callerThreadId },
              ),
            );
          }
            ...
            .pipe(
              Effect.mapError(
                (error) =>
                  new GatewayToolError(
                    "caller_turn_inactive",
                    "This Luminor write was rejected because the caller thread could no longer be verified.",
          ...
          if (
            ...
            return yield* Effect.fail(
              new GatewayToolError(
                "caller_turn_inactive",
                "This Luminor write was rejected because the turn that received this MCP request is no longer active. In-flight requests cannot inherit authority from a later turn.",
                {
                  callerThreadId,
