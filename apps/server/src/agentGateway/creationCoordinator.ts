...
export const makeCreateThreadsHandler = Effect.fn(function* (
  ...
  const awaitCreationReplay = (
    ...
    Effect.gen(function* () {
      ...
      return yield* Effect.fail(
        new GatewayToolError(
          "operation_failed",
          "The original thread-creation operation is still in progress. Retry only with the same request id; Luminor will not create replacement threads.",
  ...
  const appendThreadCreationRecap = (input: {
    ...
    readonly result: LuminorCreateThreadsResult;
  }) => {
    ...
    return orchestrationEngine
      .dispatch({
        ...
        activity: {
          ...
          summary: `Created ${input.result.createdCount} Luminor ${threadLabel}`,
          payload: {
            source: "luminor_mcp",
            operationId: input.result.operationId,
  ...
  const run = (input: typeof LuminorCreateThreadsInput.Type, context: GatewayCreationContext) => {
    return Effect.gen(function* () {
      ...
      if (deprecatedBranchName) {
        return yield* Effect.fail(
          new ToolInputError(
            '"branchName" is no longer supported for managed worktrees. Luminor creates a managed temporary branch and renames it after the first prompt; create additional branches inside the new thread if needed.',
          ),
        );
      }
        ...
        Effect.gen(function* () {
          ...
          if (environment === "worktree") {
            ...
            if (existsSync(plannedWorktreePath)) {
              return yield* Effect.fail(
                new ToolInputError(
                  `Worktree path "${plannedWorktreePath}" already exists. Luminor will not reuse or remove a pre-existing path.`,
                ),
              );
            }
          }
        ...
        Effect.gen(function* () {
                          ...
                          Effect.flatMap(() =>
                            worktree.branch === null
                              ...
                              : // The branch is this operation's own deterministic
                                // luminor/* name and its worktree was just force-removed.
          ...
          if (compensationErrors.length > 0) {
            ...
            return new GatewayToolError(
              "operation_failed",
              "Luminor could not dispatch the exact creation plan and cleanup is still pending. The durable operation remains compensating and will never create replacements.",
          ...
          if (statusFailure !== null) {
            ...
            return new GatewayToolError(
              "operation_failed",
              "Luminor compensated the created resources but could not persist a terminal operation status. The operation remains compensating and will never create replacements.",
              { operationId, ...failure, compensationPending: true },
            );
          }
          return new GatewayToolError(
            "operation_failed",
            "Luminor could not dispatch the exact creation plan. Created operation-owned resources were compensated; no replacements were created.",
            { operationId, ...failure },
          );
        });
      ...
      const outcome = yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
                ...
                Effect.gen(function* () {
                  ...
                  yield* orchestrationEngine
                    .dispatch({
                      ...
                      worktreePath,
                      creationSource:
                        context.kind === "external-client" ? "external_mcp" : "luminor_mcp",
                      ...(context.kind === "provider-session"
                        ? {
          ...
          const result = {
            ...
            threads: results,
          } satisfies LuminorCreateThreadsResult;
