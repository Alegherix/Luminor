...
async function fetchLocalPageTitleResponse(
  ...
  const response = await globalThis.fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "LuminorLocalServerMonitor/1.0",
    },
  });
...
function toServerProcess(
  ...
  return {
    ...
    addresses,
    isStoppable,
    ...(isStoppable ? {} : { stopDisabledReason: "Luminor cannot signal this process." }),
  };
}
...
export async function stopLocalServer(
  ...
  if (!target.isStoppable) {
    return {
      pid: input.pid,
      stopped: false,
      message: target.stopDisabledReason ?? "Luminor cannot stop this process.",
    };
  }
