...
function makeConfig(overrides: Partial<ServerConfigShape> = {}): ServerConfigShape {
  const baseDir = makeTempDir("luminor-effect-http-");
  return {
    mode: "web",
    port: 0,
    host: "127.0.0.1",
...
describe("production Effect HTTP routes", () => {
  it("preserves the loopback-only startup-token policy", () => {
    ...
    expect(
      isLegacyTokenAuthorized({
        config: { ...loopback, publicUrl: new URL("https://luminor.example.test/") },
        url: new URL("http://127.0.0.1/attachments/id?token=desktop-secret"),
      }),
    ).toBe(false);
    expect(
    ...
    expect(
      isLegacyTokenAuthorized({
        config: loopback,
        url: new URL("http://100.64.1.20/attachments/id?token=desktop-secret"),
    ...
    await withEffectServer(
      ...
      async (origin) => {
        const requests: ReadonlyArray<RequestInit | undefined> = [
          ...
          { method: "POST", headers: { Cookie: "luminor_session=browser-session" } },
        ];
  ...
  it("keeps the route unavailable outside a private desktop loopback deployment", async () => {
    ...
    const unsafeConfigs: ReadonlyArray<Partial<ServerConfigShape>> = [
      ...
      { mode: "desktop", publicUrl: new URL("https://luminor.example.test/") },
  ...
  it("preserves dev redirect, static file, and SPA fallback behavior", async () => {
    ...
    const staticDir = makeTempDir("luminor-effect-static-");
    mkdirSync(path.join(staticDir, "assets"), { recursive: true });
    writeFileSync(path.join(staticDir, "index.html"), "<main>Luminor shell</main>");
    writeFileSync(path.join(staticDir, "assets", "app.js"), "globalThis.luminor = true;");
    await withEffectServer(makeConfig({ staticDir }), { kind: "static" }, async (origin) => {
      const asset = await fetch(`${origin}/assets/app.js`);
      expect(asset.status).toBe(200);
      await expect(asset.text()).resolves.toContain("globalThis.luminor");

      const fallback = await fetch(`${origin}/chat/thread-id`);
      expect(fallback.status).toBe(200);
      expect(fallback.headers.get("content-type")).toContain("text/html");
      await expect(fallback.text()).resolves.toContain("Luminor shell");
    });
  });
  ...
  it("serves precompressed sidecars by Accept-Encoding with identity fallback", async () => {
    const staticDir = makeTempDir("luminor-effect-static-");
    mkdirSync(path.join(staticDir, "assets"), { recursive: true });
    const source = "globalThis.luminor = true;".repeat(64);
  ...
  it("refuses symlinks that escape the static root", async () => {
    const parentDir = makeTempDir("luminor-effect-static-");
  ...
  it("marks hashed assets immutable and keeps index.html revalidating", async () => {
    ...
    mkdirSync(path.join(staticDir, "assets"), { recursive: true });
    writeFileSync(path.join(staticDir, "index.html"), "<main>Luminor shell</main>");
    writeFileSync(path.join(staticDir, "assets", "app-abc123.js"), "globalThis.luminor = true;");
    ...
    await withEffectServer(makeConfig({ staticDir }), { kind: "static" }, async (origin) => {
      ...
      for (const traversal of [
        ...
        if (response.status === 200) {
          expect(response.body, traversal).toContain("Luminor shell");
        }
      }
    });
  });
