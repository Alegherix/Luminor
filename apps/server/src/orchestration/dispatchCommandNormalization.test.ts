...
function projectCreateCommand(
  ...
  return {
    ...
    kind: "chat",
    title: "Chat",
    workspaceRoot: "/Users/tester/Documents/Luminor/2026-06-11/chat",
...
describe("makeDispatchCommandNormalizer", () => {
  it("returns a deferred prepare effect instead of scaffolding during normalization", async () => {
    ...
    expect(preparedRoots).toEqual(["/Users/tester/Documents/Luminor/2026-06-11/chat"]);
  });
  ...
  it("does not prepare ordinary projects or the chat workspace root itself", async () => {
    ...
    const first = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          workspaceRoot: "/Users/tester/Documents/Luminor/2026-06-11/app",
        }),
      }),
    );
    ...
    const second = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          workspaceRoot: "/Users/tester/Documents/Luminor",
        }),
      }),
    );
    ...
    const result = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "studio",
          title: "Studio",
          workspaceRoot: "/Users/tester/Documents/Luminor/Studio",
        }),
      }),
    );
    await runPrepareWorkspaceRoot(result);

    expect(preparedRoots).toEqual(["/Users/tester/Documents/Luminor/Studio"]);
  });
  ...
  it("prepares nested Studio workspace roots but not ordinary projects under Studio", async () => {
    ...
    const first = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "studio",
          workspaceRoot: "/Users/tester/Documents/Luminor/Studio/Outbox",
        }),
      }),
    );
    await runPrepareWorkspaceRoot(first);
    const second = await Effect.runPromise(
      normalizer({
        command: projectCreateCommand({
          kind: "project",
          workspaceRoot: "/Users/tester/Documents/Luminor/Studio/SomeProject",
        }),
      }),
    );
    await runPrepareWorkspaceRoot(second);

    expect(preparedRoots).toEqual(["/Users/tester/Documents/Luminor/Studio/Outbox"]);
  });

  it("defers binary attachment authority to the transactional managed ledger", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "luminor-dispatch-normalize-"));
