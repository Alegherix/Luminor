...
const TestServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "luminor-orchestration-engine-test-",
});
  ...
  it("stores completed checkpoint summaries even when no files changed", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        checkpointRef: asCheckpointRef("refs/luminor/checkpoints/thread-turn-diff/turn/1"),
        status: "ready",
        files: [],
  ...
  it("rejects Studio and regular projects claiming each other's workspace root", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/luminor-cross-kind-studio",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    await system.run(
      engine.dispatch({
        ...
        kind: "project",
        title: "App",
        workspaceRoot: "/tmp/luminor-cross-kind-app",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    ...
    await expect(
      system.run(
        engine.dispatch({
          ...
          projectId: asProjectId("project-cross-kind-app"),
          kind: "studio",
          workspaceRoot: "/tmp/luminor-cross-kind-pinned-studio",
        }),
      ),
  ...
  it("rejects moving a Studio container onto another Studio workspace root", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/luminor-studio-source",
        defaultModelSelection: null,
        createdAt,
      }),
    );
    await system.run(
      engine.dispatch({
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/tmp/luminor-studio-target",
        defaultModelSelection: null,
        createdAt,
      }),
    );
