...
export interface ThreadSubscriptionDeps<E> extends OrchestrationSubscriptionDeps<E> {
  readonly resumeFromSequence?: number | undefined;
  readonly resumeSubjectExists?: Effect.Effect<boolean, E> | undefined;
}
...
export function makeThreadSubscriptionStream<E>(
  threadId: ThreadId,
  deps: ThreadSubscriptionDeps<E>,
): Stream.Stream<SnapshotLiveStreamItem<ThreadDetailSnapshotResult>, E | WsRpcError, Scope.Scope> {
  const { orchestrationEngine, snapshotQuery, toError } = deps;
  return makeCursorSafeSnapshotLiveStream({
    ...(deps.resumeFromSequence === undefined
      ? {}
      : { resumeFromSequence: deps.resumeFromSequence }),
    ...(deps.resumeSubjectExists === undefined
      ? {}
      : { resumeSubjectExists: deps.resumeSubjectExists }),
    ...(deps.onResnapshotRequired ? { onResnapshotRequired: deps.onResnapshotRequired } : {}),
    subscribeLive: orchestrationEngine.subscribeDomainEvents.pipe(
      Effect.map((stream) =>
        deps.bufferLive(
