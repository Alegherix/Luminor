...
// Purpose: Build a ZIP archive that exports a single Luminor thread so a user
...
// The export-eligibility guard lives in @luminor/shared/threadExport so the
// web composer and the HTTP route share one predicate.

import zlib from "node:zlib";

import type { OrchestrationMessage, OrchestrationThread } from "@luminor/contracts";
...
export function threadArchiveFileName(input: {
  ...
  return `luminor-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
