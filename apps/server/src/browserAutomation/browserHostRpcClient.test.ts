...
const TEST_CAPABILITY = "luminor-browser-host-client-test-capability-0123456789";

function pipePathForTest(name: string): string {
  return process.platform === "win32"
    ? String.raw`\\.\pipe\luminor-${name}-${process.pid}-${crypto.randomUUID()}`
    : path.join("/tmp", `luminor-${process.pid}-${crypto.randomUUID().slice(0, 8)}.sock`);
}
...
describe("browser host RPC client", () => {
  ...
  it("prefers the canonical host path and accepts the legacy alias during upgrade", () => {
    expect(
      resolveBrowserHostPipePath({
        LUMINOR_BROWSER_HOST_PIPE_PATH: "/tmp/canonical.sock",
        LUMINOR_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock",
      }),
    ).toBe("/tmp/canonical.sock");
    expect(resolveBrowserHostPipePath({ LUMINOR_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock" })).toBe(
      "/tmp/legacy.sock",
    );
  });
  ...
  it("accepts only a bounded private desktop capability from direct test environments", () => {
    expect(
      resolveBrowserHostCapability({
        LUMINOR_BROWSER_HOST_CAPABILITY: TEST_CAPABILITY,
      }),
