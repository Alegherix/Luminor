...
const originalLuminorStaticDir = process.env.LUMINOR_STATIC_DIR;

function makeTempDir(prefix = "luminor-config-test-"): string {
...
afterEach(() => {
  ...
  tempDirs.clear();
  if (originalLuminorStaticDir === undefined) {
    delete process.env.LUMINOR_STATIC_DIR;
  } else {
    process.env.LUMINOR_STATIC_DIR = originalLuminorStaticDir;
  }
});

describe("resolveStaticDir", () => {
  it("uses the desktop static snapshot exposed through the Luminor environment", async () => {
    const snapshotDir = makeTempDir("luminor-static-snapshot-test-");
    fs.writeFileSync(path.join(snapshotDir, "index.html"), "<main>Luminor</main>");
    process.env.LUMINOR_STATIC_DIR = snapshotDir;
...
describe("resolveDefaultChatWorkspaceRoot", () => {
  it("places the managed chat workspace under Documents/Luminor on macOS and Linux", () => {
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Luminor");
    expect(
      resolveDefaultChatWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Luminor");
  });
  ...
  it("uses Windows separators when deriving the managed chat workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Luminor");
  });
  ...
  it("defaults to the current process platform when no platform is supplied", () => {
    ...
    try {
      expect(resolveDefaultChatWorkspaceRoot({ homeDir: "C:\\Users\\tester" })).toBe(
        "C:\\Users\\tester\\Documents\\Luminor",
      );
    } finally {
...
describe("resolveDefaultStudioWorkspaceRoot", () => {
  it("places the Studio workspace under Documents/Luminor/Studio on macOS and Linux", () => {
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Documents/Luminor/Studio");
    expect(
      resolveDefaultStudioWorkspaceRoot({
        homeDir: "/home/tester",
        platform: "linux",
      }),
    ).toBe("/home/tester/Documents/Luminor/Studio");
  });
  ...
  it("uses Windows separators when deriving the Studio workspace on Windows", () => {
    ...
    ).toBe("C:\\Users\\tester\\Documents\\Luminor\\Studio");
  });
});
...
describe("resolveCanonicalWorkspaceRoots", () => {
  it("canonicalizes a symlinked home directory to match project row realpaths", async () => {
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedHomeDir, "Documents", "Luminor"));
    expect(result.studioWorkspaceRoot).toBe(
      path.join(expectedHomeDir, "Documents", "Luminor", "Studio"),
    );
  });
  ...
  it("canonicalizes the nearest existing ancestor when the workspace root itself does not exist yet", async () => {
    ...
    // Luminor/ nor Luminor/Studio exist yet underneath it.
    ...
    expect(result.chatWorkspaceRoot).toBe(path.join(expectedDocuments, "Luminor"));
    expect(result.studioWorkspaceRoot).toBe(path.join(expectedDocuments, "Luminor", "Studio"));
