...
const luminorSkillPath = "/Users/me/.luminor/skills/reviewer/SKILL.md";
...
describe("shouldInlineSkillForProvider", () => {
  it("skips codex-native and luminor roots for codex but inlines foreign provider roots", () => {
    // Codex loads .codex roots natively and ~/.luminor/skills via the extra
    // skill root registered at session start.
    expect(shouldInlineSkillForProvider("codex", luminorSkillPath)).toBe(false);
  ...
  it("inlines only Luminor-owned paths for cursor", () => {
    expect(shouldInlineSkillForProvider("cursor", luminorSkillPath)).toBe(true);
  ...
  it("inlines everything except .claude paths for claudeAgent", () => {
    ...
    expect(shouldInlineSkillForProvider("claudeAgent", luminorSkillPath)).toBe(true);
  ...
  it("inlines cross-provider paths for pi but not pi-native skills", () => {
    expect(shouldInlineSkillForProvider("pi", luminorSkillPath)).toBe(true);
  ...
  it("always inlines for providers without native skill support", () => {
    for (const provider of ["antigravity", "grok", "kilo", "opencode"] as const) {
      expect(shouldInlineSkillForProvider(provider, luminorSkillPath)).toBe(true);
...
describe("buildInlineSkillInstructions", () => {
  it("inlines skill content for non-native providers and skips unreadable paths", async () => {
    ...
    const skillDir = path.join(root, ".luminor", "skills", "reviewer");
    try {
      ...
      const text = await buildInlineSkillInstructions({
        ...
        skills: [
          ...
          { name: "missing", path: path.join(root, ".luminor", "skills", "missing", "SKILL.md") },
        ],
        maxChars: 10_000,
      });
  ...
  it("does not inline luminor-rooted skills for codex (covered by the extra skill root)", async () => {
    const text = await buildInlineSkillInstructions({
      provider: "codex",
      skills: [{ name: "reviewer", path: luminorSkillPath }],
      maxChars: 10_000,
    });
    expect(text).toBe("");
  });
