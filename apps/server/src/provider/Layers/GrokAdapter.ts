...
import { decodeOutboundJson, decodeOutboundText, outboundHttp } from "@luminor/shared/outboundHttp";
import {
  Cause,
  DateTime,
  Deferred,
...
export const takeGrokLuminorHarnessPolicyTextPart = (
...
const GROK_ACP_DEBUG_ENV = "LUMINOR_GROK_ACP_DEBUG";
const LUMINOR_GROK_ACP_DEBUG_ENV = "LUMINOR_GROK_ACP_DEBUG";
...
// override with LUMINOR_GROK_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const GROK_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "LUMINOR_GROK_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const GROK_PLAN_GUARD_HOOK_CALLBACK_ID = "luminor-plan-guard";
...
export function buildGrokPromptMeta(interactionMode: ProviderInteractionMode): {
  ...
  // the provider state when Luminor sends the desired mode again.
...
export function resolveGrokPlanHookResponse(
  ...
  return {
    decision: "deny",
    systemMessage: `Luminor Plan mode blocks the mutating or unknown Grok tool "${toolName || "unknown"}".`,
  };
}
...
function isGrokAcpDebugEnabled(): boolean {
  return (
    ...
    process.env[LUMINOR_GROK_ACP_DEBUG_ENV] === "1" ||
...
export function makeGrokAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // present, every session gets the luminor_* MCP tools.
                ...
                Effect.gen(function* () {
                  ...
                  if (planMarkdown !== undefined) {
                    ...
                    if (
                      ...
                      ctx.lastPlanFingerprint = planMarkdown;
                      // The extension response must reach Grok before Luminor cancels the
    ...
    const startGrokTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeGrokLuminorHarnessPolicyTextPart(
          ctx,
              ...
              Effect.gen(function* () {
                ...
                if (terminalPlanMarkdown !== undefined) {
                  ...
                  yield* offerRuntimeEvent(ctx.lifecycleGeneration, {
                    ...
                    raw: {
                      source: "acp.jsonrpc",
                      method: "luminor.grok.terminal-plan-response",
                      payload: result,
                    },
                  });
                }
