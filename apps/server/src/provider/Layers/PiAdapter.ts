...
import { takeLuminorHarnessPolicyForProviderSession } from "../../agentGateway/harnessPolicy.ts";
import {
...
// a native clipboard module, so importing it during Luminor startup can bloat the
  ...
  if (isRecord(result) && result.isError === true) {
    const message = Array.isArray(result.content)
      ? result.content
          ...
          .join("\n")
      : "";
    throw new Error(message || "Luminor gateway tool failed.");
  }
  const content =
...
export async function buildPiAgentGatewayCustomTools(input: {
  ...
  if (tools.length === 0) {
    throw new Error("Luminor MCP returned an empty tool catalog.");
  }
 ...
 * before it crosses Luminor's trimmed-string RPC contract. A single malformed
...
const makePiAdapter = (options?: PiAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    // Bridges the common Pi extension UI primitives onto Luminor's existing
    ...
    const makePiExtensionUIContext = (context: PiSessionContext): ExtensionUIContext => {
      ...
      const warnUnsupported = (method: string) => {
        ...
        offerRuntimeEvent({
          ...
          payload: {
            message: `Pi extension UI API '${method}' is not supported in Luminor yet.`,
            detail: { method },
          },
          raw: {
            source: "pi.sdk.event",
      ...
      const uiContext: ExtensionUIContext = {
        ...
        setTheme() {
          return { success: false, error: "Luminor does not expose Pi themes." };
        },
        getToolsExpanded() {
          return false;
        },
      ...
      Effect.gen(function* () {
          ...
          ? yield* releaseAgentGatewaySessionLeaseOnInterrupt(
              ...
              Effect.catch((cause) =>
                Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                  Effect.andThen(
                    Effect.logWarning(
                      "Pi could not install thread-scoped Luminor gateway tools",
                      cause,
                    ),
                  ),
        ...
        if (loadedExtensions.length > 0) {
          ...
          offerRuntimeEvent({
            ...
            payload: {
              message:
                "Pi extensions are loaded with Luminor's limited UI bridge. select/confirm/input/notify/status are supported; TUI-only widgets and editor hooks are ignored.",
              detail: {
    ...
    const sendTurn: PiAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeLuminorHarnessPolicyForProviderSession(context, {
          provider: PROVIDER,
    ...
    const respondUnsupported = (threadId: ThreadId, method: string) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method,
          detail: `Pi does not expose Luminor approval/user-input requests for thread ${threadId}.`,
        }),
      );
