...
import { ServerProviderUpdateError } from "@luminor/contracts";
import { parseCodexConfigModelProvider } from "@luminor/shared/codexConfig";
import { decodeJsonResult } from "@luminor/shared/schemaJson";
...
const DISABLED_PROVIDER_STATUS_MESSAGE = "Provider is disabled in Luminor settings.";
...
function parseCursorAuthStatusFromOutput(result: CommandResult): {
  ...
  if (result.code === 0) {
    return {
      status: "warning",
      authStatus: "unknown",
      message: "Cursor Agent is installed, but Luminor could not verify authentication status.",
    };
  }
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      ...(hasApiKey
        ...
        : {
            message:
              "Droid CLI is installed. Luminor can use the CLI's cached device-pairing login; run `droid` to authenticate locally if needed, or set FACTORY_API_KEY.",
          }),
    } satisfies ServerProviderStatus;
  });
...
export const checkPiProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    // Pi itself is SDK-backed in Luminor. Keep this CLI probe advisory so health
    ...
    if (versionProbe.outcome === "missing" || versionProbe.outcome === "failure") {
      ...
      return {
        ...
        message:
          versionProbe.outcome === "missing"
            ? "Pi SDK is bundled, but the Pi CLI (`pi`) is not on PATH, so Luminor could not verify the installed CLI version."
    ...
    if (versionProbe.outcome === "timeout") {
      return {
        ...
        checkedAt,
        message:
          "Pi SDK is bundled, but the CLI health check timed out before Luminor could verify the installed version.",
      } satisfies ServerProviderStatus;
    }
    ...
    return {
      ...
      message: configuredAgentDir
        ? `Pi CLI is installed. Luminor will use Pi agent dir ${configuredAgentDir}.`
  ...
  Effect.gen(function* () {
    ...
    if (
      ...
      return {
        ...
        version: parsedVersion,
        checkedAt,
        message: `Antigravity CLI ${parsedVersion} is too old for Luminor. Upgrade to ${MINIMUM_ANTIGRAVITY_CLI_VERSION} or newer.`,
      } satisfies ServerProviderStatus;
    }
  ...
  Effect.gen(function* () {
    ...
    if (Option.isNone(modelsProbe.success)) {
      return {
        ...
        checkedAt,
        message:
          "Cursor Agent is authenticated, but model discovery timed out before Luminor could verify available models.",
      } satisfies ServerProviderStatus;
    }
      ...
      const updateProvider: ProviderHealthShape["updateProvider"] = Effect.fn(
        ...
        if (!isProviderEnabledForSettings(provider, settings)) {
          return yield* new ServerProviderUpdateError({
            provider,
            reason: "Provider is disabled in Luminor settings.",
          });
        }
          ...
          const finalProviders = yield* setProviderUpdateState(
            ...
            makeUpdateState({
              ...
              message: stillOutdated
                ? `Update command completed, but Luminor still detects an outdated provider version${stillOutdatedVersions}.`
                : "Provider updated.",
              output: output ? output.slice(0, UPDATE_OUTPUT_MAX_BYTES) : null,
            }),
          );
