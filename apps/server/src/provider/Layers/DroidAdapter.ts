...
import { buildAcpLuminorMcpServers } from "../../agentGateway/mcpInjection.ts";
import {
...
export const takeDroidLuminorHarnessPolicyTextPart = (
...
const DROID_ACP_DEBUG_ENV = "LUMINOR_DROID_ACP_DEBUG";
...
// override with LUMINOR_DROID_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const DROID_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "LUMINOR_DROID_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const DROID_PLAN_MODE_PROMPT_PREFIX = [
  "Luminor Droid plan mode is active.",
...
interface DroidSessionContext {
  ...
  /** Turns cancelled by Luminor only because their Plan proposal was captured. */
  ...
  return Effect.gen(function* () {
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeDroidAcpRuntime({
            ...
            clientInfo: { name: "Luminor", version: "0.0.0" },
            ...(agentGatewayCredentials
              ? {
          ...
          if (resumeSessionId !== undefined && started.sessionSetupMethod === "new") {
            return yield* new ProviderAdapterRequestError({
              ...
              detail:
                "Droid could not resume the requested native session. Luminor refused the fresh fallback to avoid silently losing conversation context.",
            });
          }
    ...
    const startDroidTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeDroidLuminorHarnessPolicyTextPart(
          ctx,
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Droid ACP version does not advertise session/fork; Luminor will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Droid session has a turn in flight; Luminor will rebuild the fork from its retained transcript.",
          });
        }
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeDroidAcpRuntime({
                ...
                clientInfo: { name: "Luminor Fork", version: "0.0.0" },
              });
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Luminor Model Discovery",
          });
          yield* runtime.start();
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Luminor Command Discovery",
          });
          yield* runtime.start();
