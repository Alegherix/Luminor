...
describe("Antigravity CLI integration helpers", () => {
  it("rotates the gateway lease per print turn and rejects a retained prior bootstrap", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-turn-lease-"));
    ...
    try {
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const bootstrapA = spawnedEnvironments[0]?.LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
          ...
          const bootstrapB = spawnedEnvironments[1]?.LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
  ...
  it("installs the generated Luminor MCP plugin alongside the capture hooks", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-home-test-"));
    const stdioProxy = {
      command: "/Applications/Luminor.app/Contents/MacOS/Luminor",
      ...
      expect(
        ...
        mcpServers: {
          luminor: {
            ...
            env: {
              LUMINOR_AGENT_GATEWAY_URL: "$LUMINOR_AGENT_GATEWAY_URL",
              LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "$LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN",
  ...
  it("gives an Antigravity turn only its thread-scoped gateway credential", () => {
    const env = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        ...
        LUMINOR_AGENT_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        LUMINOR_AGENT_GATEWAY_TOKEN: "stale-token",
        LUMINOR_AUTH_TOKEN: "host-control-plane-token",
        LUMINOR_BROWSER_HOST_PIPE_PATH: "/tmp/desktop.sock",
        LUMINOR_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock",
    ...
    expect(env).toEqual({
      ...
      HOME: "/home/test",
      GEMINI_API_KEY: "gemini-key",
      LUMINOR_AGENT_GATEWAY_URL: "http://127.0.0.1:3773/mcp",
      LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "thread-a-bootstrap",
      LUMINOR_ANTIGRAVITY_EVENTS: "/tmp/thread-a-hooks.ndjson",
      LUMINOR_ANTIGRAVITY_HOOK_DECISION: "allow",
    });
  });
  ...
  it("advertises canonical browser tools only while the session owns a gateway lease", () => {
    ...
    expect(identityOnlyPrompt).toContain("Luminor MCP control is unavailable");
    ...
    const envWithoutLease = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        LUMINOR_AGENT_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        LUMINOR_AGENT_GATEWAY_TOKEN: "stale-token",
        LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "stale-bootstrap",
      },
    });
    expect(envWithoutLease.LUMINOR_AGENT_GATEWAY_URL).toBeUndefined();
    expect(envWithoutLease.LUMINOR_AGENT_GATEWAY_TOKEN).toBeUndefined();
    expect(envWithoutLease.LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN).toBeUndefined();
  });
  ...
  it("keeps the globally installed hook neutral outside Luminor sessions", () => {
    const command = buildAntigravityCaptureCommand(
      "__luminor_gui_must_not_launch__",
  ...
  it("answers pre-tool with a decision from the capture script when capture is inactive", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-hook-test-"));
    ...
    try {
      ...
      const result = spawnSync(process.execPath, [scriptPath, "pre-tool"], {
        env: { ...process.env, LUMINOR_ANTIGRAVITY_EVENTS: "" },
  ...
  it("runs the capture script for Luminor-managed sessions", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-hook-test-"));
    const scriptPath = path.join(directory, "capture.cjs");
    ...
    try {
      ...
      const result = runCaptureCommand(command, payload, {
        LUMINOR_ANTIGRAVITY_EVENTS: eventPath,
        LUMINOR_ANTIGRAVITY_HOOK_DECISION: "allow",
      });
  ...
  it("runs packaged Electron as Node only for Luminor-managed sessions", () => {
    expect(
      buildAntigravityCaptureCommand(
        "/Applications/Luminor.app/Contents/MacOS/Luminor",
        "/tmp/luminor-capture/capture.cjs",
        "pre-tool",
        "darwin",
      ),
    ).toBe(
      `if [ -z "\${LUMINOR_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '{"decision":"ask"}'; else ELECTRON_RUN_AS_NODE=1 '/Applications/Luminor.app/Contents/MacOS/Luminor' '/tmp/luminor-capture/capture.cjs' 'pre-tool'; fi`,
    );
    expect(
      buildAntigravityCaptureCommand(
        String.raw`C:\Program Files\Luminor\Luminor.exe`,
        String.raw`C:\Users\test\.gemini\capture.cjs`,
        "pre-tool",
        "win32",
      ),
    ).toBe(
      String.raw`if not defined LUMINOR_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo {"decision":"ask"}) else (set "ELECTRON_RUN_AS_NODE=1" && "C:\Program Files\Luminor\Luminor.exe" "C:\Users\test\.gemini\capture.cjs" "pre-tool")`,
    );
  });
  ...
  it("advances file offsets only past complete JSONL records", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-test-"));
  ...
  it("streams hook tool names and terminal states without arguments", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-tool-events-"));
    ...
    const spawnProcess = ((
      ...
      eventFile = options.env?.LUMINOR_ANTIGRAVITY_EVENTS;
  ...
  it("answers stop hooks with a neutral allow-exit payload", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-stop-hook-"));
    const scriptPath = path.join(directory, "capture.cjs");
    const eventPath = path.join(directory, "events.ndjson");
    try {
      await fs.writeFile(scriptPath, hookScriptSource(), { mode: 0o700 });
      const result = spawnSync(process.execPath, [scriptPath, "stop"], {
        env: { ...process.env, LUMINOR_ANTIGRAVITY_EVENTS: eventPath },
...
describe("Antigravity turn settle on cancel (#465)", () => {
  ...
  it("unlocks Cancel without letting a late close settle the follow-up", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-interrupt-hung-"));
