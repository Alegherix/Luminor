...
import { TurnId } from "@luminor/contracts";
...
import {
  ...
  takeGrokLuminorHarnessPolicyTextPart,
} from "./GrokAdapter.ts";

describe("Grok Luminor harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeGrokLuminorHarnessPolicyTextPart(state, true)?.text).toContain(
      LUMINOR_HARNESS_POLICY_MARKER,
    );
    expect(takeGrokLuminorHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
...
describe("Grok native plan approval", () => {
  it("adds Plan instructions without sending a pager-only slash command as model text", () => {
    ...
    ).toMatch(/^Luminor requested Grok's native plan mode\./u);
  });
  ...
  it("backs native Plan mode with a fail-closed pre-tool hook", () => {
    expect(
      resolveGrokPlanHookResponse("plan", {
        hookCallbackId: "luminor-plan-guard",
  ...
  it("keeps native plan mode gated after Luminor captures the plan", () => {
    expect(makeGrokExitPlanModeCapturedResponse()).toEqual({
      outcome: "cancelled",
      feedback:
        "Luminor captured this plan for user review. Do not revise or implement it now. End this turn and wait for the user's next message.",
    });
  });
...
describe("Grok native user questions", () => {
  ...
  it("maps Luminor answers to Grok's question-text keyed response", () => {
