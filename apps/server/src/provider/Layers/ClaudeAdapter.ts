...
import { buildClaudeSubagentPrompt } from "@luminor/shared/agentMentions";
...
import { renderLuminorHarnessPolicy } from "../../agentGateway/harnessPolicy.ts";
...
// from these hints (see extractSubagentIdentityHints in @luminor/shared/subagents).
...
export const buildEmbeddedClaudeSystemPromptAppend = (gatewayControlAvailable: boolean) =>
  [
    "You are running inside Luminor, a coding app that embeds the Claude Agent SDK.",
    ...
    renderLuminorHarnessPolicy({ gatewayControlAvailable }),
  ].join("\n");
        ...
        if (Exit.isFailure(exit)) {
          if (hasPendingUserInterrupt(context) || isClaudeInterruptedCause(exit.cause)) {
            ...
            if (isClaudeMissingResumeConversationCause(exit.cause)) {
              ...
              // starts a fresh Claude session and bootstraps Luminor's retained
    ...
    const startSessionUnlocked: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const settings = {
          ...
          // Luminor's safer 200k budget explicit unless the thread opts into 1M.
