...
describe("Claude Luminor harness policy", () => {
  it("advertises scoped MCP additively when credentials are available", () => {
    const text = buildEmbeddedClaudeSystemPromptAppend(true);
    assert.include(text, LUMINOR_HARNESS_POLICY_MARKER);
    assert.include(text, "Use the luminor_* tools");
    assert.notInclude(text, "Luminor MCP control is unavailable");
  });

  it("stays truthful when scoped MCP credentials are absent", () => {
    const text = buildEmbeddedClaudeSystemPromptAppend(false);
    assert.include(text, LUMINOR_HARNESS_POLICY_MARKER);
    assert.include(text, "Luminor MCP control is unavailable");
  });
});
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("injects the canonical Luminor browser MCP into an Opus 4.8 session", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        systemPrompt.append ?? "",
        "exact thread-scoped Electron page Luminor surfaces to the user",
      );
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
  ...
  it.effect("loads Claude filesystem settings sources for SDK sessions", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(systemPrompt.append ?? "", LUMINOR_HARNESS_POLICY_MARKER);
      assert.include(systemPrompt.append ?? "", "Luminor is the host and harness");
      // This characterization harness intentionally omits gateway credentials.
      assert.include(systemPrompt.append ?? "", "Luminor MCP control is unavailable");
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
    );
...
`;
  ...
  it.effect("sets plan permission mode on sendTurn when interactionMode is plan", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(promptText ?? "", "Luminor plan mode is active.");
