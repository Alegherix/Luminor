...
import {
  ...
  LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV,
  LUMINOR_AGENT_GATEWAY_URL_ENV,
} from "../../agentGateway/mcpInjection.ts";
import {
  type LuminorHarnessPolicyDeliveryState,
...
/**
 * Hook output when capture is inactive (the session is not Luminor-managed).
...
export function buildAntigravityCaptureCommand(
  ...
  if (platform === "win32") {
    return `if not defined LUMINOR_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo ${fallback}) else (set "ELECTRON_RUN_AS_NODE=1" && ${invocation})`;
  }
  return `if [ -z "\${LUMINOR_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '${fallback}'; else ELECTRON_RUN_AS_NODE=1 ${invocation}; fi`;
}
...
process.stdin.on("end", () => {
  const target = process.env.LUMINOR_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.LUMINOR_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    ...
    `${JSON.stringify(
      {
        ...
        name: "luminor-capture",
        description: "Streams Antigravity CLI lifecycle events to Luminor when requested.",
      },
      null,
      2,
    )}\n`,
...
export function buildAntigravityTurnProcessEnvironment(input: {
  ...
  const gatewayKeys = hasGatewayBootstrap
    ? [LUMINOR_AGENT_GATEWAY_URL_ENV, LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]
    : [];
  const gatewayEnvironment = hasGatewayBootstrap
    ? {
        [LUMINOR_AGENT_GATEWAY_URL_ENV]: input.gatewayConnection!.url,
        [LUMINOR_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: input.gatewayBootstrapToken!,
      }
    : {};
  return buildProviderChildEnvironment({
    provider: PROVIDER,
    ...(input.baseEnv === undefined ? {} : { baseEnv: input.baseEnv }),
    inheritedLuminorKeys: [
      "LUMINOR_ANTIGRAVITY_EVENTS",
      "LUMINOR_ANTIGRAVITY_HOOK_DECISION",
      ...gatewayKeys,
    ],
    overrides: {
      LUMINOR_ANTIGRAVITY_EVENTS: input.eventFile,
...
export function buildAntigravityTurnPrompt(
  ...
  const harnessPolicy = takeLuminorHarnessPolicyForProviderSession(state, {
    provider: PROVIDER,
        ...
        yield* Effect.tryPromise({
          try: () =>
            ...
            new ProviderAdapterRequestError({
              ...
              detail: messageFromCause(cause, "Failed to install the Luminor capture hook."),
              cause,
            }),
        });
    ...
    const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const runDir = yield* Effect.tryPromise({
          try: () => fs.mkdtemp(path.join(os.tmpdir(), "luminor-antigravity-")),
          catch: (cause) =>
            new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "turn/prepare",
        ...
        if (gatewaySessionLease && !gatewayBootstrapToken) {
          ...
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
            detail: "The Luminor gateway credential is no longer active for this provider turn.",
          });
        }
