...
//          that makes Luminor catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@luminor/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    ...
    case "codex":
      ...
      // folders plus `~/.luminor/skills`, which Luminor registers at session start
    ...
    case "cursor":
      ...
      // only Luminor-owned paths need inlining.
      return segments.has(".luminor");
    case "claudeAgent":
