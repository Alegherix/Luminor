...
describe("claudeProcessEnv", () => {
  ...
  it("does not grant Luminor control-plane authority to Claude", () => {
    const result = buildClaudeProcessEnv({
      env: {
        ANTHROPIC_API_KEY: "api-key-auth",
    ...
    assert.equal(result.ANTHROPIC_API_KEY, "api-key-auth");
    assert.equal(result.LUMINOR_AUTH_TOKEN, undefined);
    assert.equal(result.LUMINOR_BROWSER_USE_PIPE_PATH, undefined);
