...
//          walking) plus the unified cross-provider skills catalog backing Luminor
//          portable skills. Aggregates `~/.luminor/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureLuminorSkillsDir
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Luminor base dir (usually `~/.luminor`); skills live in `{base}/skills`. */
  readonly luminorBaseDir: string;
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Luminor. */
  readonly includeLuminorRoot?: boolean;
}
...
const ensuredLuminorSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredLuminorSkillsDirs.clear();
}

export function luminorSkillsDir(luminorBaseDir: string): string {
  return nodePath.join(luminorBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureLuminorSkillsDir(luminorBaseDir: string): Promise<string> {
  const dir = luminorSkillsDir(luminorBaseDir);
  if (ensuredLuminorSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredLuminorSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  luminor: {
    homeRoots: (input) => [luminorSkillsDir(input.luminorBaseDir)],
    projectRootNames: [".luminor"],
  },
  codex: {
    // Keep Luminor's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Luminor as the
...
function orderedOriginsForProvider(
  provider: ProviderKind | null | undefined,
  includeLuminorRoot = true,
  includeRemainingOrigins = true,
): SkillsHomeOrigin[] {
  const preferred = preferredOriginsForProvider(provider);
  const ordered = [...preferred];
  if (includeLuminorRoot && !ordered.includes("luminor")) {
    ordered.push("luminor");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeLuminorRoot || origin !== "luminor");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeLuminorRoot && origin === "luminor") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeLuminorRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureLuminorSkillsDir(input.luminorBaseDir);
    const roots = [
