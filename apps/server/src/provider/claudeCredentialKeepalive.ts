...
//
// Opt in:   LUMINOR_CLAUDE_KEEPALIVE=1
// Tune:     LUMINOR_CLAUDE_KEEPALIVE_MINUTES=<n>   (default 30)
...
export function isClaudeCredentialKeepaliveEnabled(
  ...
  return platform === "darwin" && envFlagEnabled(env.LUMINOR_CLAUDE_KEEPALIVE);
}
...
export function resolveClaudeCredentialKeepaliveIntervalMs(env: NodeJS.ProcessEnv): number {
  const raw = env.LUMINOR_CLAUDE_KEEPALIVE_MINUTES?.trim();
...
export function startClaudeCredentialKeepalive(input?: {
  ...
  // Luminor should not do it as background work merely because the app opened.
