 ...
 */
import { type GrokModelOptions, type RuntimeMode } from "@luminor/contracts";
...
export function buildGrokAcpSpawnInput(
  ...
  // process, while native Plan mode plus Luminor's pre-tool hook still gate
  ...
  Effect.gen(function* () {
    ...
    if (!hasApiKey && authMethodIds.has(GROK_API_KEY_AUTH_METHOD_ID)) {
      return yield* new AcpErrors.AcpRequestError({
        code: -32602,
        errorMessage:
          "Grok ACP requires API-key authentication, but XAI_API_KEY is not set. Set XAI_API_KEY and restart Luminor, or run `grok login` to create a cached login.",
    ...
    if (hasApiKey && !authMethodIds.has(GROK_API_KEY_AUTH_METHOD_ID)) {
      return yield* new AcpErrors.AcpRequestError({
        code: -32602,
        errorMessage: `Grok did not advertise API-key authentication even though XAI_API_KEY is set (advertised: ${advertised}). Update Grok or check its login policy, then restart Luminor.`,
        data: { authMethods: [...authMethodIds], reason: "compatibility_mismatch" },
      });
    }
    return yield* new AcpErrors.AcpRequestError({
      code: -32602,
      errorMessage: `Grok ACP advertised no supported headless authentication method (advertised: ${advertised}). Luminor supports cached_token and xai.api_key; update Grok and retry.`,
      data: {
