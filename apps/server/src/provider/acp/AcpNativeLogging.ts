import type { ProviderKind, ThreadId } from "@luminor/contracts";
...
const SENSITIVE_ENTRY_NAMES = new Set(["authorization", "luminor_agent_gateway_token"]);

function redactSecretString(value: string): string {
  return value
    .replace(/(\bBearer\s+)[^\s"',}\]]+/gi, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(/(LUMINOR_AGENT_GATEWAY_TOKEN\s*=\s*)[^\s"',}\]]+/g, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(/("LUMINOR_AGENT_GATEWAY_TOKEN"\s*:\s*")[^"]*/g, `$1${ACP_LOG_REDACTED_VALUE}`)
    .replace(
      /("name"\s*:\s*"LUMINOR_AGENT_GATEWAY_TOKEN"\s*,\s*"value"\s*:\s*")[^"]*/g,
