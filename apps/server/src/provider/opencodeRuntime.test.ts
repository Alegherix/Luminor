...
import type { ChatAttachment } from "@luminor/contracts";
import { resolveWindowsComSpec } from "@luminor/shared/windowsProcess";
...
describe("toOpenCodeFileParts", () => {
  it("materializes image attachments as SDK file parts", () => {
    ...
    expect(
      toOpenCodeFileParts({
        ...
        resolveAttachmentPath: () => "/tmp/luminor-attachments/screenshot.png",
      }),
    ).toEqual([
      {
        type: "file",
        ...
        url: "file:///tmp/luminor-attachments/screenshot.png",
      },
    ]);
  });
  ...
  it("leaves generic files for prompt-path projection", () => {
    ...
    expect(
      toOpenCodeFileParts({
        attachments: [attachment],
        resolveAttachmentPath: () => "/tmp/luminor-attachments/notes.docx",
      }),
    ).toEqual([]);
  });
});
...
describe("buildOpenCodeServerProcessEnv", () => {
  ...
  it("strips inherited Luminor authority from managed server processes", () => {
    const env = buildOpenCodeServerProcessEnv({
      baseEnv: {
        OPENAI_API_KEY: "provider-key",
        LUMINOR_AUTH_TOKEN: "server-secret",
        LUMINOR_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
      },
    });

    expect(env.OPENAI_API_KEY).toBe("provider-key");
    expect(env.LUMINOR_AUTH_TOKEN).toBeUndefined();
        ...
        Effect.gen(function* () {
          ...
          const first = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "luminor-thread-a",
            })
            .pipe(Effect.provideService(Scope.Scope, firstScope));
          const second = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "luminor-thread-b",
            })
            .pipe(Effect.provideService(Scope.Scope, secondScope));
