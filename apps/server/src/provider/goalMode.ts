// FILE: goalMode.ts
// Purpose: Injects Luminor's provider-independent persistent thread objective.
...
Before claiming completion, inspect the current state and verify every requirement against authoritative evidence. When the full objective is complete, call luminor_set_thread_goal with achieved: true before ending the turn so Luminor can stop the continuation loop and record the achievement.

If the same external blocker prevents meaningful progress for three consecutive goal turns, call luminor_set_thread_goal with blocked: true so Luminor pauses the goal instead of looping. Do not mark the goal blocked merely because the work is difficult, incomplete, or would benefit from clarification.`;
}
