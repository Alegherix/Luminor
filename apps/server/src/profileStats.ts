...
// Purpose: Compute Profile-page stats from Luminor's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@luminor/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored Luminor user message, plus the
...
>()("luminor/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Luminor prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from Luminor's own DB (no external
  ...
  const getProfileStats = (
    ...
    Effect.gen(function* () {
      ...
      // ── Identity ──
      const homeDirBasename = nodePath.basename(config.homeDir) || "luminor";
