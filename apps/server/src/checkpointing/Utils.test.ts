...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Luminor refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/luminor\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/luminor/", "refs/historical/");
