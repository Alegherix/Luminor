  ...
  const captureCheckpointOnce: CheckpointStoreShape["captureCheckpoint"] = (input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "luminor-fs-checkpoint-" }),
        (tempDir) =>
          Effect.gen(function* () {
            ...
            const commitEnv: NodeJS.ProcessEnv = {
              ...
              GIT_AUTHOR_NAME: "Luminor",
              GIT_AUTHOR_EMAIL: "luminor@users.noreply.github.com",
              GIT_COMMITTER_NAME: "Luminor",
              GIT_COMMITTER_EMAIL: "luminor@users.noreply.github.com",
            };
            ...
            const message = `Luminor checkpoint ref=${input.checkpointRef}`;
  ...
  const reverseCheckpointDiff: CheckpointStoreShape["reverseCheckpointDiff"] = (input) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "luminor-checkpoint-undo-" }),
        (tempDir) =>
          Effect.gen(function* () {
