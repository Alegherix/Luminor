...
import { CheckpointRef } from "@luminor/contracts";
...
describe("CheckpointStoreLive", () => {
  ...
  it("deduplicates concurrent captures for the same checkpoint ref", async () => {
    ...
    await runtime.runPromise(
      Effect.gen(function* () {
        ...
        const input = {
          cwd: "/repo",
          checkpointRef: CheckpointRef.makeUnsafe("refs/luminor-checkpoints/thread/message"),
        };
  ...
  it("seeds a capture from the working index so Git can reuse its stat cache", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "luminor-checkpoint-index-test-"));
      ...
      await runtime.runPromise(
        Effect.gen(function* () {
          ...
          yield* store.captureCheckpoint({
            cwd: tempDir,
            checkpointRef: CheckpointRef.makeUnsafe("refs/luminor-checkpoints/thread/stat-cache"),
          });
        }),
      );
  ...
  it("skips the capture when skipIfExists is set and the ref already exists", async () => {
    const existingRef = "refs/luminor-checkpoints/thread/existing";
    const missingRef = "refs/luminor-checkpoints/thread/missing";
    const execute = vi.fn<GitCoreShape["execute"]>((input) => {
      const args = input.args.join(" ");
  ...
  it("restores the worktree patch when resetting the index fails during file undo", async () => {
    const fromRef = CheckpointRef.makeUnsafe("refs/luminor-checkpoints/thread/turn/start");
    const toRef = CheckpointRef.makeUnsafe("refs/luminor-checkpoints/thread/turn/end");
  ...
  it("fails when a checkpoint ref cannot be deleted", async () => {
    const lockedRef = CheckpointRef.makeUnsafe("refs/luminor/checkpoints/thread/turn/locked");
    const deletableRef = CheckpointRef.makeUnsafe("refs/luminor/checkpoints/thread/turn/ok");
    const execute = vi.fn<GitCoreShape["execute"]>((input) => {
      const args = input.args.join(" ");
  ...
  it("tolerates deleting checkpoint refs that are already absent", async () => {
    ...
    const missingRef = CheckpointRef.makeUnsafe("refs/luminor/checkpoints/thread/turn/gone");
