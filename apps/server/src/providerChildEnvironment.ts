...
// Purpose: Builds provider child environments without Luminor control-plane authority.
...
const isTestHarnessKey = (key: string, env: NodeJS.ProcessEnv): boolean =>
  Boolean(env.VITEST) && (key.startsWith("LUMINOR_FAKE_") || key.startsWith("LUMINOR_ACP_"));

export function buildProviderChildEnvironment(input: {
  readonly provider: ProviderChildKind;
  readonly baseEnv?: NodeJS.ProcessEnv;
  readonly inheritedLuminorKeys?: ReadonlyArray<string>;
  ...
  const allowedLuminorKeys = new Set(input.inheritedLuminorKeys ?? []);
  ...
  for (const [key, value] of Object.entries(baseEnv)) {
    if (
      key.startsWith("LUMINOR_") &&
      !allowedLuminorKeys.has(key) &&
