...
export const LUMINOR_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveLuminorCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.LUMINOR_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".luminor", "runtime");
  return path.join(overlayRoot, LUMINOR_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Luminor keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveLuminorCodexHomeOverlayPath(env, source);
