...
import { createCachedLoginShellPathReader } from "@luminor/shared/loginShellEnvironment";
import {
  expandHomePath as expandHomePathSync,
  resolveLuminorHomeDirectory,
} from "@luminor/shared/luminorHome";
...
export function fixPath(
  ...
  try {
    ...
    // Cached by default: the probe result is persisted under the Luminor home and reused
...
export const resolveBaseDir = (raw: string | undefined): Effect.Effect<string> =>
  Effect.succeed(resolveLuminorHomeDirectory({ configuredHome: raw }));
