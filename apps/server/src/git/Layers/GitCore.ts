...
import { parseGitHubRepositoryNameWithOwnerFromRemoteUrl } from "@luminor/shared/githubRepository";
...
const AUTO_DETACHED_WORKTREE_DIRNAME = "luminor";
const WORKTREE_OWNERSHIP_MARKER = "luminor-agent-gateway-owner.json";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `luminor-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
  ...
  Effect.gen(function* () {
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempIndexDir = yield* fileSystem.makeTempDirectoryScoped({
            prefix: `luminor-git-status-index-${process.pid}-`,
          });
      ...
      Effect.gen(function* () {
        ...
        if (patch.length > 0) {
          yield* Effect.acquireUseRelease(
            Effect.tryPromise({
              try: () => nodeFs.mkdtemp(nodePath.join(tmpdir(), "luminor-worktree-patch-")),
              catch: (cause) =>
                createGitCommandError(
    ...
    const removeWorktree: GitCoreShape["removeWorktree"] = (input) =>
      Effect.gen(function* () {
        ...
        // luminor/* branches qualify for reclamation; detached HEADs and
    ...
    const stashAndCheckout: GitCoreShape["stashAndCheckout"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["stash", "push", "-u", "-m", `luminor: stash before switching to ${input.branch}`],
          {
            timeoutMs: 30_000,
