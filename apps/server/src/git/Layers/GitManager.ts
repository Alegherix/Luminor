...
} from "@luminor/shared/git";
import { parseGitHubRepositoryNameWithOwnerFromRemoteUrl } from "@luminor/shared/githubRepository";
import { summarizeUnifiedPatchTotals } from "@luminor/shared/unifiedPatchStats";
import { resolveWorktreeHandoffIntent } from "@luminor/shared/worktreeHandoff";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `luminor/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.gen(function* () {
  ...
  const runPrStep = (
    ...
    Effect.gen(function* () {
      ...
      const bodyFile = path.join(tempDir, `luminor-pr-body-${process.pid}-${randomUUID()}.md`);
      yield* fileSystem
  ...
  const handoffThread: GitManagerShape["handoffThread"] = Effect.fnUntraced(function* (input) {
    ...
    if (input.targetMode === "local") {
      ...
      const preservedLocalStash = yield* stashWorkingTree(
        input.cwd,
        `luminor preserve local handoff ${randomUUID()}`,
      );
      const sourceStash = yield* stashWorkingTree(
        input.worktreePath,
        `luminor handoff to local ${randomUUID()}`,
      );
    ...
    const sourceStash = yield* stashWorkingTree(
      input.cwd,
      `luminor handoff to worktree ${randomUUID()}`,
    );
