...
function prepareWindowsLaunch(assetsDirectory: string, extractionRoot: string): LaunchCommand {
  ...
  const executables = findFiles(applicationRoot, (candidate) =>
    /[/\\]Luminor\.exe$/i.test(candidate),
  );
  if (executables.length !== 1) {
    throw new Error(`Expected one extracted Luminor.exe, found ${executables.length}.`);
  }
...
export function createPackagedDesktopSmokeEnvironment(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LUMINOR_HOME: join(root, "luminor-home"),
  ...
  if (options.platform === "mac") {
    const userDataPath = join(env.HOME!, "Library", "Application Support", "luminor");
...
export async function verifyPackagedDesktopStartup(
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), `luminor-packaged-smoke-${options.platform}-`));
  ...
  try {
    ...
    const logPath = join(env.LUMINOR_HOME!, "userdata", "logs", "desktop-main.log");
