// FILE: canary.ts
// Purpose: Maintains and launches an isolated, frozen Luminor Canary checkout.
...
export function resolveCanaryPaths(
  ...
  const home = Path.resolve(
    env.LUMINOR_CANARY_HOME?.trim() || Path.join(homeDirectory, ".luminor-canary"),
  );
  const cacheBase = env.XDG_CACHE_HOME?.trim() || Path.join(homeDirectory, ".cache");
  const source = Path.resolve(
    env.LUMINOR_CANARY_SOURCE?.trim() || Path.join(cacheBase, "luminor-canary", "source"),
  );
  return {
    home,
    source,
...
function assertManagedSourceIsClean(paths: CanaryPaths): void {
  ...
  if (status.length > 0) {
    throw new Error(
      `Luminor Canary's managed source has tracked local changes. Refusing to overwrite ${paths.source}.`,
    );
  }
}
...
function startCanary(paths: CanaryPaths): void {
  ...
  if (existingPid !== null && isRunning(existingPid)) {
    console.log(`Luminor Canary is already running (pid ${String(existingPid)}).`);
    return;
  }
  ...
  if (
    ...
    throw new Error("Luminor Canary is not built. Run `bun run canary:setup` first.");
  }
  FS.mkdirSync(paths.home, { recursive: true });
  const env = { ...process.env };
  delete env.VITE_DEV_SERVER_URL;
  delete env.ELECTRON_RENDERER_PORT;
  delete env.LUMINOR_AUTH_TOKEN;
  Object.assign(env, {
    LUMINOR_DESKTOP_FLAVOR: "canary",
    LUMINOR_DISABLE_AUTO_UPDATE: "1",
    LUMINOR_HOME: paths.home,
    LUMINOR_COMMIT_HASH: commit,
  });
  ...
  try {
    ...
    if (child.pid === undefined) {
      throw new Error("Luminor Canary failed to return a process id.");
    }
    child.unref();
    FS.writeFileSync(paths.pid, `${String(child.pid)}\n`, { mode: 0o600 });
    console.log(`Started Luminor Canary at ${commit.slice(0, 12)} (pid ${String(child.pid)}).`);
...
function rollbackCanary(paths: CanaryPaths): void {
  ...
  if (state?.previousCommit === null || state?.previousCommit === undefined) {
    throw new Error("Luminor Canary has no previous successful commit to restore.");
  }
  assertManagedSourceIsClean(paths);
  stopCanary(paths);
...
function printStatus(paths: CanaryPaths): void {
  ...
  console.log(`Luminor Canary: ${running ? `running (pid ${String(pid)})` : "stopped"}`);
