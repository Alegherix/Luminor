...
const originsAttribution = `Luminor began as a clone of [${firstDisplayName}](https://github.com/pingdotgg/${firstName}), but it has since become a substantially different product with its own branding, packaging, release system, provider orchestration, desktop app behavior, and product direction.`;
const releaseAttribution = `**A review of the Luminor codebase found an analytics configuration that came from the original ${firstSpacedDisplayName} codebase when Luminor was created as a clone in March. We did not add it, and we have no access to the PostHog project receiving the events.**`;
const inAppReleaseAttribution = `"A review of the Luminor codebase found an analytics configuration that came from the original ${firstSpacedDisplayName} codebase when Luminor was created as a clone in March.",`;

describe("brand identity guard", () => {
  it("detects retired names in paths and text", () => {
    const violations = findBrandIdentityViolations([
      { path: `docs/${firstName}.md`, contents: "Luminor" },
      { path: "source.ts", contents: `const value = "${secondName}:state";` },
    ]);
    expect(violations).toHaveLength(2);
  });

  it("does not match ordinary numeric type names or canonical Luminor text", () => {
    expect(
      findBrandIdentityViolations([
        { path: "source.ts", contents: "const value = new Uint32Array(); // Luminor" },
      ]),
    ).toEqual([]);
  });
  ...
  it("requires user-facing raster assets to match a visually approved digest", () => {
    const approvedContents = new TextEncoder().encode("approved Luminor screenshot");
    const approvedDigest = "3bd4c5f818e880c431cee50bb4649fddc11beb9e09df8266d3287238c9f5f7a9";
