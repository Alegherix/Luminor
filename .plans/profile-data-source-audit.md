...
You are GPT-5.5 (xhigh) in the Luminor monorepo at `/Users/emanueledipietro/Developer/luminor`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `LUMINOR_HOME` (dev: `./.luminor/electron-dev`; prod: `~/.luminor` or
  `~/Documents/Luminor`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `LUMINOR_HOME/LUMINOR_HOME/LUMINOR_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.luminor/electron-dev/dev/state.sqlite` (dev) and `~/.luminor/userdata/state.sqlite`
     (prod) where relevant.
