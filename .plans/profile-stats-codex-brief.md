...
You are GPT-5.5 (xhigh) working in the Luminor monorepo at `/Users/emanueledipietro/Developer/luminor`.
...
- BRANCH dev DB (what the dev instance uses): `./.luminor/electron-dev/dev/state.sqlite`  (~40 turns)
- REAL app DB: `~/.luminor/userdata/state.sqlite`  (~437 turns, 192 MB) — the important one
...
2. `npx turbo run typecheck --filter=@luminor/contracts --filter=@luminor/cli` must pass. DO NOT run the web
   (`@luminor/web`) typecheck — its UI is being rewritten in parallel and will be red until Opus finishes.
