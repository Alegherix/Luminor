# SYN-47 — Luminor Studio (head-to-toe implementation plan)
...
Add a **"Studio"** experience to Luminor that is a near-exact clone of the existing **Chats / Threads**
...
- Studio chats are **normal AI threads** (full chat surface, any provider) — but their working directory is
  rooted at **`~/Documents/Luminor/Studio`**, so anything the agent writes lands there, e.g.
  `~/Documents/Luminor/Studio/Outbox/Content/2026-06-09_luminor_local_dev_server_x_posts.md`.
...
2. **Folders:** Mirror `~/Documents/Claude/*` structure under `~/Documents/Luminor/Studio`. Single shared
...
- A hidden project with `kind: "chat"` rooted at `chatWorkspaceRoot` (= `~/Documents/Luminor`) acts as the
...
- A hidden **Studio container** project (`kind: "studio"`) rooted at **`studioWorkspaceRoot`**
  (= `~/Documents/Luminor/Studio`). Lazily ensured by a new `ensureStudioProject`.
...
of `~/Documents/Luminor/Studio` with **no extra change** to this resolver. (Codex CLI/Claude SDK then run with
...
- No required change: `resolveThreadWorkspaceCwd` only special-cases `"chat"`. Confirm
  `@luminor/shared/threadWorkspace` helpers (`isWorkspaceRootWithin`, `workspaceRootsEqual`) are exported for
...
1. **`workspaceStore.ts`** — add `studioWorkspaceRoot: string | null` to state + a setter; extend
   `setServerWorkspacePaths` to accept/store it. **Bump the persist key** (`luminor:workspace-pages:v2` → `v3`)
...
- **Manual smoke (for the UI-refinement pass / verify skill):**
  ...
  3. Start a studio chat, ask it to write a file → it appears under `~/Documents/Luminor/Studio/Outbox/Content/…`.
...
- **cwd correctness:** verify a studio thread's resolved cwd is `~/Documents/Luminor/Studio` (not scratch/tmp).
...
3. A new studio chat is a real AI thread whose cwd is `~/Documents/Luminor/Studio`; agent-written files land
