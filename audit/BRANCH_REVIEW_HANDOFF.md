# Luminor Code-Quality Branch — Complete Review Handoff
...
- Effect remains in Luminor for lifecycle, cancellation, queues, errors, and application policy.
...
AcpSessionRuntime
  ...
  └─ Luminor: lifecycle, bounds, logging, policy, normalized events
        │
        ▼
...
Use the official TypeScript SDK for the protocol wire, while keeping Effect for Luminor's application
lifecycle.
...
3. Provider adapters should depend on a stable Luminor runtime interface, not choose a protocol
   implementation.
...
The package currently retains only generated/schema/error adapters still imported by Luminor. A
...
SQLite persistence, or the full Luminor app.
...
```text
Review the Luminor branch `codex/audit-code-quality` as a senior architecture, correctness,
