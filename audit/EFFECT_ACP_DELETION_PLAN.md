...
process supervision, teardown, and Luminor lifecycle policy. The official
...
2. `effect` for Luminor runtime and lifecycle ownership.
3. Two small local modules beside the ACP runtime:
   - `AcpErrors.ts`, containing only the Effect-native errors Luminor still consumes;
...
- `AcpErrors.ts` owns only Luminor's three Effect-native runtime errors, and `AcpExtensions.ts` owns
...
1. A minimal Effect codec for the parts of `SessionConfigOption` Luminor reads.
...
3. Delete the redundant Effect-Schema decode of `session/update`; the official SDK already validates
   it before invoking Luminor's handler.
...
7. Normalize SDK `null` values at consumption sites where Luminor uses `undefined`; do not introduce
...
- Effect still owns Luminor runtime/lifecycle policy;
